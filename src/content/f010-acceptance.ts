...
/**
 * F010（梶井基次郎3作品追加）work単位atomic受入の薄い橋渡しラッパー。
 *
 * DD-F010.mdどおり、F010のvoiced段階はF008/F009と同じ間接ハッシュ規約
 * （voiceEvidenceRef・capacity-actual evidenceファイルのSHA-256だけをstage record
 * outputHashesへ格納する方式、scripts/f010-prepare-voice.ts / scripts/
 * f010-capacity-actual.ts参照）を採用している。汎用src/content/batch-acceptance.ts
 * のpromoteVerifiedWorkArtifactsが要求する直接evidence値（generationDigest・
 * completenessDigest等）は、scripts/f010-capacity-actual.tsが同じmanifest work上へ
 ...
 * src/content/f009-acceptance.tsと同じ理由・同じパターンのF010向け複製。
...
async function verifyPreviewTree(root: string, preview: IntegratedBuild): Promise<void> {
  ...
  if (!isAbsolute(preview.stagingRoot) || !relation || relation === '..' ||
    ...
    return fail('F010_ACCEPTANCE_PREVIEW_MISMATCH', 'preview stagingがworkspace外です');
  }
  const files: Array<{ path: string; bytes: Uint8Array }> = [];
  const walk = async (current: string, logical: string): Promise<void> => {
    const info = await lstat(current);
    if (info.isSymbolicLink()) return fail('F010_ACCEPTANCE_PREVIEW_MISMATCH', 'preview treeにlinkがあります');
    if (info.isFile()) {
      files.push({ path: logical, bytes: await readFile(current) });
      return;
    }
    if (!info.isDirectory()) return fail('F010_ACCEPTANCE_PREVIEW_MISMATCH', 'preview treeが不正です');
  ...
  if (actual !== preview.buildSha256 || canonicalJson(declared) !== canonicalJson(recomputed)) {
    return fail('F010_ACCEPTANCE_PREVIEW_MISMATCH', 'preview tree/hash/bytesを再計算できません');
  }
}

export class F010AcceptanceError extends Error {
  constructor(public readonly code: string, message: string) {
    super(message);
    this.name = 'F010AcceptanceError';
  }
}

export interface PreparedF010WorkAcceptance {
  readonly __brand: 'PreparedF010WorkAcceptance';
...
async function canonical<T>(root: string, path: string): Promise<T> {
  ...
  if (canonicalJson(value) !== text) return fail('F010_ACCEPTANCE_ARTIFACT', `${path}がcanonical JSONではありません`);
  return value;
}
...
async function loadManifest(root: string): Promise<BatchManifest> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F010') {
    return fail('F010_ACCEPTANCE_MANIFEST', 'F010 manifestが不正です');
  }
...
function assertOrder(manifest: BatchManifest, workId: WorkId): void {
  ...
  if (index < 0 || !work || !['voiced', 'accepted'].includes(work.status) ||
    ...
    fail('F010_WORK_ORDER', 'manifest順の先行accepted＋現在work条件を満たしません');
  }
}

/**
 * F010間接ハッシュ規約下のcanonical allowlistから、promoteVerifiedWorkArtifactsが
 * 要求する直接evidenceを再読込・再検証し、偽造不能なprepared値をmintする。
 * @des DES-F010-009 @fun FUN-F010-010
 */
export async function prepareF010WorkAcceptance(
  workspace: string,
  manifestPath: string,
  workId: WorkId | string,
): Promise<PreparedF010WorkAcceptance> {
  const root = await workspaceRoot(workspace);
  if (manifestPath !== MANIFEST_PATH || !WORK_ID.test(workId)) {
    return fail('F010_ACCEPTANCE_PATH', 'manifest/work pathがallowlist外です');
  }
  const id = workId as WorkId;
  const manifest = await loadManifest(root);
  assertOrder(manifest, id);
  const cache = `.cache/batch-accept/F010/${id}`;
  const [
    voiceEvidence,
    generationCanonical,
    completeness,
    ...
    canonical<Record<string, unknown>>(root, `content/batches/F010/voice-evidence/${id}.json`),
    canonical<VoiceDiffGenerationResult>(root, `content/batches/F010/work-artifacts/${id}/voice-generation.json`),
    canonical<VoiceCompletenessReport>(root, `content/batches/F010/work-artifacts/${id}/voice-completeness.json`),
    canonical<ActualCapacityReport>(root, `content/batches/F010/capacity-actual/${id}.json`),
  ...
  if (
    voiceEvidence.batchId !== 'F010' || voiceEvidence.workId !== id ||
    ...
    generation.batchId !== 'F010' || generation.workId !== id ||
    completeness.batchId !== 'F010' || completeness.workId !== id || completeness.result !== 'pass' ||
    actual.batchId !== 'F010' || actual.workId !== id || !['pass', 'pass_with_warning'].includes(actual.result) ||
    preview.mode !== 'work-preview' || preview.activeBatchId !== 'F010' || preview.activeWorkId !== id ||
    pages.batchId !== 'F010' || pages.workId !== id ||
    ...
    work?.actualCapacityRef !== `content/batches/F010/capacity-actual/${id}.json`
  ) {
    return fail('F010_ACCEPTANCE_PREVIEW_MISMATCH', 'canonical voice/capacity/preview tupleが一致しません');
  }
  await verifyPreviewTree(root, preview);
  const prepared = Object.freeze({
    __brand: 'PreparedF010WorkAcceptance' as const,
...
function assertPrepared(prepared: PreparedF010WorkAcceptance, expectedManifestSha: Sha256 | string): void {
  if (!preparedValues.has(prepared) || prepared.__brand !== 'PreparedF010WorkAcceptance') {
    fail('F010_ACCEPTANCE_PREPARED_UNTRUSTED', 'production prepareがmintした値ではありません');
  }
  if (prepared.expectedManifestSha !== expectedManifestSha) {
    fail('F010_ACCEPTANCE_MANIFEST_STALE', 'expected manifest SHAがprepare時点と一致しません');
  }
}

async function promote(
  workspace: string,
  prepared: PreparedF010WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions,
  dependencies: F010AcceptanceDependencies,
): Promise<F010AcceptanceResult> {
...
/**
 * generic accepted-audio transactionをF010 prepared brandに限定して呼ぶ。
 * @des DES-F010-009 @fun FUN-F010-010
 */
export async function acceptF010Work(
  workspace: string,
  prepared: PreparedF010WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions = {},
  dependencies: F010AcceptanceDependencies = productionDependencies,
): Promise<F010AcceptanceResult> {
 ...
 * @des DES-F010-009 @fun FUN-F010-010
 */
export async function recoverF010WorkAcceptance(
  workspace: string,
  prepared: PreparedF010WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions = {},
