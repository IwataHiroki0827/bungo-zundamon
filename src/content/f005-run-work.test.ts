...
describe('F005 production work runner', () => {
  ...
  it('build logと混在しても一意marker付き1行JSONから結果を抽出できる', () => {
    ...
    const activeLeaseDirectoryCode =
      'F005_ETW_PID_NOT_JOB_MEMBER_SYSTEM_DIRECTORY_ACTIVE_LEASE_WRITE_REJOIN_CANDIDATE';
    const activeLeaseDirectory = new F005NativeCapacityError(
      activeLeaseDirectoryCode,
      'fixed active lease directory stage only',
    );
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: activeLeaseDirectory }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
        code: activeLeaseDirectoryCode,
      },
    });
    const privateActiveLeaseDirectory = Object.assign(new Error('private'), {
      name: 'F005NativeCapacityError',
      code:
    ...
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: privateActiveLeaseDirectory }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
    ...
    const boundFileObjectCode =
      'F005_ETW_PID_NOT_JOB_MEMBER_SYSTEM_PROCESS_BOUND_FILE_OBJECT_REJOIN_CANDIDATE';
    const boundFileObject = new F005NativeCapacityError(
      boundFileObjectCode,
      'fixed bound FileObject stage only',
    );
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: boundFileObject }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
        code: boundFileObjectCode,
      },
    });
    const privateBoundFileObject = Object.assign(new Error('private'), {
      name: 'F005NativeCapacityError',
      code:
    ...
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: privateBoundFileObject }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
    ...
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: legacyBoundFileObject }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
    ...
    const boundLeaseDirectoryCode =
      'F005_ETW_PID_NOT_JOB_MEMBER_SYSTEM_DIRECTORY_BOUND_LEASE_WRITE_REJOIN_CANDIDATE';
    const boundLeaseDirectory = new F005NativeCapacityError(
      boundLeaseDirectoryCode,
      'fixed bound lease directory stage only',
    );
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: boundLeaseDirectory }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
        code: boundLeaseDirectoryCode,
      },
    });
    const privateBoundLeaseDirectory = Object.assign(new Error('private'), {
      name: 'F005NativeCapacityError',
      code:
    ...
    expect(JSON.parse(
      formatF005RunnerFailure(
        new Error('voice boundary', { cause: privateBoundLeaseDirectory }),
      ).slice(F005_RUNNER_FAILURE_PREFIX.length),
    )).toMatchObject({
      cause: {
        name: 'F005NativeCapacityError',
    ...
    expect(F005_WRITE_COMPLETION_DRAIN_FAILURE_STAGES).toHaveLength(103);
    ...
    for (const stage of F005_WRITE_COMPLETION_DRAIN_FAILURE_STAGES) {
      ...
      const isAmbiguity = stage ===
        ...
        stage === 'COMPLETED_NO_LEASE_DIRECTORY_HANDOFF_IDENTITY_MATCH_NONE' ||
        stage === 'COMPLETED_NO_LEASE_DIRECTORY_HANDOFF_IDENTITY_MATCH_AMBIGUOUS' ||
