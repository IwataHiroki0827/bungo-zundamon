...
/**
 * F008（江戸川乱歩3作品追加）work単位atomic受入の薄い橋渡しラッパー。
 *
 * DD-F008.md FUN-F008-010どおり、F008のvoiced段階はF007と同じ間接ハッシュ規約
 * （voiceEvidenceRef・capacity-actual evidenceファイルのSHA-256だけをstage record
 * outputHashesへ格納する方式、scripts/f008-prepare-voice.ts / scripts/
 * f008-capacity-actual.ts参照）を採用している。汎用src/content/batch-acceptance.ts
 * のpromoteVerifiedWorkArtifactsが要求する直接evidence値（generationDigest・
 * completenessDigest等）は、scripts/f008-capacity-actual.tsが同じmanifest work上へ
 ...
 * src/content/f007-acceptance.tsと同じ理由・同じパターンのF008向け複製。
...
async function verifyPreviewTree(root: string, preview: IntegratedBuild): Promise<void> {
  ...
  if (!isAbsolute(preview.stagingRoot) || !relation || relation === '..' ||
    ...
    return fail('F008_ACCEPTANCE_PREVIEW_MISMATCH', 'preview stagingがworkspace外です');
  }
  const files: Array<{ path: string; bytes: Uint8Array }> = [];
  const walk = async (current: string, logical: string): Promise<void> => {
    const info = await lstat(current);
    if (info.isSymbolicLink()) return fail('F008_ACCEPTANCE_PREVIEW_MISMATCH', 'preview treeにlinkがあります');
    if (info.isFile()) {
      files.push({ path: logical, bytes: await readFile(current) });
      return;
    }
    if (!info.isDirectory()) return fail('F008_ACCEPTANCE_PREVIEW_MISMATCH', 'preview treeが不正です');
  ...
  if (actual !== preview.buildSha256 || canonicalJson(declared) !== canonicalJson(recomputed)) {
    return fail('F008_ACCEPTANCE_PREVIEW_MISMATCH', 'preview tree/hash/bytesを再計算できません');
  }
}

export class F008AcceptanceError extends Error {
  constructor(public readonly code: string, message: string) {
    super(message);
    this.name = 'F008AcceptanceError';
  }
}

export interface PreparedF008WorkAcceptance {
  readonly __brand: 'PreparedF008WorkAcceptance';
...
async function canonical<T>(root: string, path: string): Promise<T> {
  ...
  if (canonicalJson(value) !== text) return fail('F008_ACCEPTANCE_ARTIFACT', `${path}がcanonical JSONではありません`);
  return value;
}
...
async function loadManifest(root: string): Promise<BatchManifest> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F008') {
    return fail('F008_ACCEPTANCE_MANIFEST', 'F008 manifestが不正です');
  }
...
function assertOrder(manifest: BatchManifest, workId: WorkId): void {
  ...
  if (index < 0 || !work || !['voiced', 'accepted'].includes(work.status) ||
    ...
    fail('F008_WORK_ORDER', 'manifest順の先行accepted＋現在work条件を満たしません');
  }
}

/**
 * F008間接ハッシュ規約下のcanonical allowlistから、promoteVerifiedWorkArtifactsが
 * 要求する直接evidenceを再読込・再検証し、偽造不能なprepared値をmintする。
 * @des DES-F008-009 @fun FUN-F008-010
 */
export async function prepareF008WorkAcceptance(
  workspace: string,
  manifestPath: string,
  workId: WorkId | string,
): Promise<PreparedF008WorkAcceptance> {
  const root = await workspaceRoot(workspace);
  if (manifestPath !== MANIFEST_PATH || !WORK_ID.test(workId)) {
    return fail('F008_ACCEPTANCE_PATH', 'manifest/work pathがallowlist外です');
  }
  const id = workId as WorkId;
  const manifest = await loadManifest(root);
  assertOrder(manifest, id);
  const cache = `.cache/batch-accept/F008/${id}`;
  const [
    voiceEvidence,
    generationCanonical,
    completeness,
    ...
    canonical<Record<string, unknown>>(root, `content/batches/F008/voice-evidence/${id}.json`),
    canonical<VoiceDiffGenerationResult>(root, `content/batches/F008/work-artifacts/${id}/voice-generation.json`),
    canonical<VoiceCompletenessReport>(root, `content/batches/F008/work-artifacts/${id}/voice-completeness.json`),
    canonical<ActualCapacityReport>(root, `content/batches/F008/capacity-actual/${id}.json`),
  ...
  if (
    voiceEvidence.batchId !== 'F008' || voiceEvidence.workId !== id ||
    ...
    generation.batchId !== 'F008' || generation.workId !== id ||
    completeness.batchId !== 'F008' || completeness.workId !== id || completeness.result !== 'pass' ||
    actual.batchId !== 'F008' || actual.workId !== id || !['pass', 'pass_with_warning'].includes(actual.result) ||
    preview.mode !== 'work-preview' || preview.activeBatchId !== 'F008' || preview.activeWorkId !== id ||
    pages.batchId !== 'F008' || pages.workId !== id ||
    ...
    work?.actualCapacityRef !== `content/batches/F008/capacity-actual/${id}.json`
  ) {
    return fail('F008_ACCEPTANCE_PREVIEW_MISMATCH', 'canonical voice/capacity/preview tupleが一致しません');
  }
  await verifyPreviewTree(root, preview);
  const prepared = Object.freeze({
    __brand: 'PreparedF008WorkAcceptance' as const,
...
function assertPrepared(prepared: PreparedF008WorkAcceptance, expectedManifestSha: Sha256 | string): void {
  if (!preparedValues.has(prepared) || prepared.__brand !== 'PreparedF008WorkAcceptance') {
    fail('F008_ACCEPTANCE_PREPARED_UNTRUSTED', 'production prepareがmintした値ではありません');
  }
  if (prepared.expectedManifestSha !== expectedManifestSha) {
    fail('F008_ACCEPTANCE_MANIFEST_STALE', 'expected manifest SHAがprepare時点と一致しません');
  }
}

async function promote(
  workspace: string,
  prepared: PreparedF008WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions,
  dependencies: F008AcceptanceDependencies,
): Promise<F008AcceptanceResult> {
...
/**
 * generic accepted-audio transactionをF008 prepared brandに限定して呼ぶ。
 * @des DES-F008-009 @fun FUN-F008-010
 */
export async function acceptF008Work(
  workspace: string,
  prepared: PreparedF008WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions = {},
  dependencies: F008AcceptanceDependencies = productionDependencies,
): Promise<F008AcceptanceResult> {
 ...
 * @des DES-F008-009 @fun FUN-F008-010
 */
export async function recoverF008WorkAcceptance(
  workspace: string,
  prepared: PreparedF008WorkAcceptance,
  expectedManifestSha: Sha256 | string,
  options: WorkPromotionOptions = {},
