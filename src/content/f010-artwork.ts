...
/**
 * F010（梶井基次郎3作品追加）専用の作者画像provenance軽量schema。
 *
 * DD-F010.md FUN-F010-014/015のとおり、F001〜F005共有のOpenAI内蔵image_gen
 * ツール専用検証（`src/notices/artwork-provenance.ts`）は一切変更・再利用
 * しない。F010の作者画像もF006〜F009と同様にローカルComfyUI（FLUX.1 schnell）
 ...
 * 作者画像を実生成し（`content/batches/F010/public-files/artwork/
 * kajii-motojiro-zundamon.png`）、既存8作者画像との非近似重複確認まで完了
 * している。本ファイルはDD-F010.mdが要求するschema/検証関数の実装。
 *
 * @des DES-F010-012 @fun FUN-F010-014 FUN-F010-015
 */
...
export type F010ArtworkErrorCode =
  | 'F010_ARTWORK_PROVENANCE_INVALID'
  | 'F010_ARTWORK_BINDING_INVALID'
  | 'F010_ARTWORK_EXISTING_INVALID'
  | 'F010_ARTWORK_NEAR_DUPLICATE';

export class F010ArtworkError extends Error {
  constructor(
    public readonly code: F010ArtworkErrorCode,
    message: string,
    options?: ErrorOptions,
  ) {
    super(message, options);
    this.name = 'F010ArtworkError';
  }
}

export interface F010ArtworkManifestIdentity {
  readonly batchId: 'F010';
  readonly authorId: '000074';
  readonly outputPath: 'artwork/kajii-motojiro-zundamon.png';
}

export interface ArtworkGenerationF010Input {
...
export interface ArtworkInputsF010 {
  readonly referenceInputs: readonly [];
}

export interface F010FinalArtworkInput {
  readonly sourcePath: 'content/batches/F010/public-files/artwork/kajii-motojiro-zundamon.png';
  readonly publicPath: 'artwork/kajii-motojiro-zundamon.png';
  readonly credit: string;
  readonly bytes: Uint8Array;
}

export interface ArtworkProvenanceF010 {
  readonly schemaVersion: '1.0.0';
  readonly manifestId: 'artwork-F010-000074-v1';
  readonly identity: F010ArtworkManifestIdentity;
  ...
  readonly output: Readonly<{
    readonly sourcePath: F010FinalArtworkInput['sourcePath'];
    readonly publicPath: F010FinalArtworkInput['publicPath'];
...
export interface ExistingArtworkInputF010 {
...
export interface ArtworkAcceptanceF010 {
  readonly result: 'pass';
  readonly path: F010FinalArtworkInput['publicPath'];
  readonly alt: '梶井基次郎をイメージしたずんだもんの独自イラスト';
  readonly sha256: Sha256;
  readonly dHash64: DHash64;
  readonly sourceProvenancePath: 'content/batches/F010/artwork-provenance.json';
  readonly provenanceSha256: Sha256;
  readonly credit: string;
...
function artworkError(
  code: F010ArtworkErrorCode,
  message: string,
  options?: ErrorOptions,
): never {
  throw new F010ArtworkError(code, message, options);
}
...
/** F005実装済みのpngjs decoderを経由してdHash64を求める。decode失敗はF010側codeへ変換する。 */
function safeComputeDHash64(bytes: Uint8Array): DHash64 {
  try {
    return computeDHash64V1(bytes);
  } catch (error) {
    if (error instanceof F005ArtworkError) {
      return artworkError('F010_ARTWORK_PROVENANCE_INVALID', `PNG decodeに失敗しました: ${error.message}`, { cause: error });
    }
    throw error;
  }
}
...
function provenancePayload(
  value: Omit<ArtworkProvenanceF010, 'provenanceSha256'>,
): Omit<ArtworkProvenanceF010, 'provenanceSha256'> {
  return value;
}

/**
 * exact final image path・参照入力exact []・実際の生成情報のみを記録した
 * ArtworkProvenanceF010をsealする。捏造した値は一切書かず、`generation`引数の
 * 実値だけをcanonical化する。
 * @des DES-F010-012 @fun FUN-F010-014 @ut UT-F010-014
 */
export function sealF010ArtworkProvenance(
  generation: ArtworkGenerationF010Input,
  inputs: ArtworkInputsF010,
  finalImage: F010FinalArtworkInput,
  ...
  if (
    ...
    return artworkError('F010_ARTWORK_PROVENANCE_INVALID', 'generation schemaが不正です');
  }
  if (
    !isRecord(inputs) || !exactKeys(inputs, ['referenceInputs']) ||
    !Array.isArray(inputs.referenceInputs) || inputs.referenceInputs.length !== 0
  ) {
    return artworkError('F010_ARTWORK_PROVENANCE_INVALID', 'referenceInputsはexact []が必要です');
  }
  if (
    !isRecord(finalImage) ||
    !exactKeys(finalImage, ['bytes', 'credit', 'publicPath', 'sourcePath']) ||
    finalImage.sourcePath !==
      'content/batches/F010/public-files/artwork/kajii-motojiro-zundamon.png' ||
    finalImage.publicPath !== 'artwork/kajii-motojiro-zundamon.png' ||
    !nonBlank(finalImage.credit) ||
    /[<>]/u.test(finalImage.credit) ||
    !(finalImage.bytes instanceof Uint8Array)
  ) {
    return artworkError('F010_ARTWORK_PROVENANCE_INVALID', 'final image schema/path/creditが不正です');
  }
...
export function serializeF010ArtworkProvenance(
  provenance: ArtworkProvenanceF010,
): string {
  if (!sealedProvenances.has(provenance) && !rehydratedProvenances.has(provenance)) {
    return artworkError('F010_ARTWORK_BINDING_INVALID', 'mintされていないprovenanceです');
  }
 ...
 */
export function parseAndRehydrateF010ArtworkProvenance(
  ...
  if (!isRecord(parsed) || canonicalJson(parsed) !== canonicalArtifact) {
    return artworkError('F010_ARTWORK_PROVENANCE_INVALID', 'provenance artifactがcanonical JSONではありません');
  }
  const expected = sealF010ArtworkProvenance(generation, inputs, finalImage);
  ...
  const restored = freezeDeep(parsed as unknown as ArtworkProvenanceF010);
  rehydratedProvenances.add(restored);
  return restored;
}

export function isMintedF010ArtworkProvenance(
  value: unknown,
): value is ArtworkProvenanceF010 {
  return isRecord(value) && (sealedProvenances.has(value) || rehydratedProvenances.has(value));
}

/**
 * 既存8作者画像を実体から再計算し、byte一致とdHash距離8以下を
 * `F010_ARTWORK_NEAR_DUPLICATE`として拒否する。authorId `000074`が既存8作者と
 * 衝突しないことも検証する。既存作者数の判定はF006がF007統合時に踏んだ実バグ
 * （`existingArtwork.length !== N`のexact一致チェックが将来の作者追加で
 * 破綻した、2026-08-22 commit `cd5ecc8`）を踏まえ、F008・F009に続き実装当初から
 * `existingArtwork.length < 8`（下限判定）とする。
 * @des DES-F010-012 @fun FUN-F010-015 @ut UT-F010-015
 */
export function verifyF010ArtworkAgainstCatalog(
  provenance: ArtworkProvenanceF010,
  finalImage: Uint8Array,
  existingArtwork: readonly ExistingArtworkInputF010[],
): ArtworkAcceptanceF010 {
  if (
    ...
    return artworkError('F010_ARTWORK_BINDING_INVALID', 'provenance/final image bindingが不正です');
  }
  // 既存作者数は下限判定（`< 8`）とし、将来のF011統合時に既存作者数が9に増えても
  // 再修正不要となるようにする（FUN-F010-014の設計判断を参照）。
  if (!Array.isArray(existingArtwork) || existingArtwork.length < 8) {
    return artworkError('F010_ARTWORK_EXISTING_INVALID', '既存8作者以上の画像が必要です');
  }
  ...
  const comparisons = existingArtwork.map((existing) => {
    ...
    if (
      ...
      !nonBlank(authorId) || authorId === '000074' ||
      ...
      return artworkError('F010_ARTWORK_EXISTING_INVALID', '既存画像identity/path/schemaが不正です');
    }
    ...
    if (actualSha256 !== claimedSha256 || actualDHash !== existing.dHash64) {
      return artworkError('F010_ARTWORK_EXISTING_INVALID', '既存画像の自己申告hashが実体と一致しません');
    }
  ...
  const acceptance = freezeDeep({
    ...
    alt: '梶井基次郎をイメージしたずんだもんの独自イラスト' as const,
    sha256: provenance.output.sha256,
    dHash64: provenance.output.dHash64,
    sourceProvenancePath: 'content/batches/F010/artwork-provenance.json' as const,
...
export function isMintedF010ArtworkAcceptance(
  value: unknown,
): value is ArtworkAcceptanceF010 {
