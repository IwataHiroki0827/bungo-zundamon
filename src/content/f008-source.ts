...
/**
 * F008（江戸川乱歩3作品追加）専用の原典・書誌・権利判定モジュール。
 *
 * DD-F008.md記載のとおり、`f005-source.ts`〜`f007-source.ts`は
 * authorId/workId定数・module-private`mintedSourceRecords`(WeakSet)の
 * brand検査が各featureへhardcodeされておりfeature非依存ではない。
 * 本モジュールは`f007-source.ts`の構造（型定義・`ProductionAozoraTransport`
 * によるHTTPS取得・権利/規約判定・書誌固定・entity正規化・決定的抽出・
 * 長大候補分割）を忠実に踏襲しつつ、authorId `001779`（江戸川乱歩）と
 * 人間椅子(056648)・Ｄ坂の殺人事件(056650)・一人二役(057193)へ
 * パラメータ化した新規実装である。
 *
 ...
 *
 * 実HTTPS取得により、人間椅子(056648)・Ｄ坂の殺人事件(056650)・一人二役(057193)の
 * 3作品XHTMLとも未定義named entityは0件（標準5種`&amp;&lt;&gt;&quot;&apos;`以外
 * 出現なし）であることを実測確認済みである（F007の舞姫`&nbsp;`のような特別contextは
 * 存在しない）。そのため本モジュールのentity正規化は3作品ともpassthroughのみを
 * 許容し、未承認named entityが1件でも出現した場合はfail-closedで拒否する。
 *
 * DOMAIN-F008.md §5が指摘するＤ坂の殺人事件（発話文字概算10,962文字/82候補）向けに、
 * F007で確立した長大候補分割（`splitOverlongF007Candidates`と同一アルゴリズム：
 * 600文字閾値・句点「。」の文境界でのみ分割・分割後order 0..N-1再採番・
 * `sourceAnchor`/`rawTokenRange`/`rawSourceSha256`は分割元と同一を維持）を
 * 実装当初から組み込む。
 *
 * @des DES-F008-004 DES-F008-005 DES-F008-015
 */

const AUTHOR_ID = '001779';
const AUTHOR_PAGE_URL = `${AOZORA_ORIGIN}/index_pages/person1779.html`;
const CANONICAL_XHTML11_DOCTYPE =
  '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">';

const F008_POLICY_IDS = Object.freeze([
  'voicevox-terms',
  'zundamon-audio-terms',
  'zundamon-character-guideline',
] as const satisfies readonly PolicyId[]);

export const F008_WORKS = Object.freeze([
  Object.freeze({
    workId: '056648',
    title: '人間椅子',
    order: 1,
    cardUrl: `${AOZORA_ORIGIN}/cards/001779/card56648.html`,
    sourceUrl: `${AOZORA_ORIGIN}/cards/001779/files/56648_58207.html`,
  }),
  Object.freeze({
    workId: '056650',
    title: 'Ｄ坂の殺人事件',
    order: 2,
    cardUrl: `${AOZORA_ORIGIN}/cards/001779/card56650.html`,
    sourceUrl: `${AOZORA_ORIGIN}/cards/001779/files/56650_58209.html`,
  }),
  Object.freeze({
    workId: '057193',
    title: '一人二役',
    order: 3,
    cardUrl: `${AOZORA_ORIGIN}/cards/001779/card57193.html`,
    sourceUrl: `${AOZORA_ORIGIN}/cards/001779/files/57193_59571.html`,
  }),
] as const);

export type F008WorkId = (typeof F008_WORKS)[number]['workId'];
export type F008Phase = 'selection' | 'predeploy';

export type F008SourceErrorCode =
  | 'F008_CONTEXT_INVALID'
  | 'F008_TRANSPORT_REQUIRED'
  | 'F008_SOURCE_RESPONSE_INVALID'
  | 'F008_SOURCE_DRIFT'
  | 'F008_USAGE_NOT_ALLOWED'
  | 'F008_BIBLIOGRAPHY_INVALID'
  | 'F008_ENTITY_NORMALIZATION_INVALID'
  | 'F008_XHTML_PREFLIGHT_REJECTED'
  | 'F008_EXTRACTION_FAILED'
  | 'F008_PATH_UNSAFE'
  | 'F008_REGISTRY_MISMATCH'
  | 'F008_CANDIDATE_SPLIT_IMPOSSIBLE';

export class F008SourceError extends Error {
  constructor(
    public readonly code: F008SourceErrorCode,
    message: string,
    options?: ErrorOptions,
  ) {
    super(message, options);
    this.name = 'F008SourceError';
  }
}

export interface F008PolicyClauseResult {
  readonly clauseId: string;
  readonly status: 'satisfied' | 'missing' | 'prohibited' | 'unknown';
}

export interface F008PolicyClauseDecision {
  readonly __brand: 'F008PolicyClauseDecision';
  readonly policyId: (typeof F008_POLICY_IDS)[number];
  ...
  readonly clauses: readonly F008PolicyClauseResult[];
}

export interface F008PolicySnapshot {
  readonly policyId: (typeof F008_POLICY_IDS)[number];
  readonly versionOrLabel: string;
  readonly artifact: RawArtifactRef;
  readonly decision: F008PolicyClauseDecision;
}

export interface F008WorkSnapshot {
...
export interface F008SourceSnapshot {
  readonly schemaVersion: '1.0.0';
  readonly authorId: '001779';
  readonly phase: F008Phase;
  ...
  readonly policies: readonly F008PolicySnapshot[];
  readonly works: readonly F008WorkSnapshot[];
}

export interface F008CollectionOptions {
  readonly policyTransport: ProductionPolicyTransport;
  readonly trustedProjectRoot: string;
  readonly workspace: string;
  readonly selectionSnapshot?: F008SourceSnapshot;
}

export interface F008UsageProfile {
...
export interface F008RightsUsageDecision {
  readonly decision: 'allow' | 'blocked';
...
export interface F008SourceRecordV2 {
  ...
  readonly bibliography: F008BibliographyV2;
}

export interface F008EntityNormalizationResult {
  readonly schemaVersion: '1.0.0';
  readonly workId: F008WorkId;
  ...
  readonly replacements: readonly never[];
}

export interface F008CandidateSet {
  readonly schemaVersion: '1.0.0';
  readonly workId: F008WorkId;
...
// ---------------------------------------------------------------------------
// 江戸川乱歩author identity・work順registry（FUN-F008-004）
// ---------------------------------------------------------------------------

const EXPECTED_REGISTRY_WORKS = Object.freeze([
  { workId: '056648', title: '人間椅子', order: 1 },
  { workId: '056650', title: 'Ｄ坂の殺人事件', order: 2 },
  { workId: '057193', title: '一人二役', order: 3 },
] as const);

export interface F008AuthorAndWorkRegistryWork {
  readonly workId: F008WorkId;
  readonly title: string;
  readonly order: 1 | 2 | 3;
}

export interface F008WorkRegistry {
  readonly __brand: 'F008WorkRegistry';
  readonly authorId: '001779';
  readonly name: 'えどがわらんぽ';
  readonly originalName: '江戸川乱歩';
  readonly slug: 'edogawa-ranpo';
  readonly identitySha256: string;
  readonly authorMode: 'introduce';
  readonly works: readonly F008AuthorAndWorkRegistryWork[];
}

export interface F008VerifiedAuthor {
  readonly __brand: 'F008VerifiedAuthor';
  readonly authorId: '001779';
  readonly name: 'えどがわらんぽ';
  readonly originalName: '江戸川乱歩';
  readonly slug: 'edogawa-ranpo';
...
const verifiedF008Authors = new WeakSet<object>();

/**
 * `authorId=001779`（江戸川乱歩）・`name=えどがわらんぽ`・`slug=edogawa-ranpo`と
 * 人間椅子(056648,order1)→Ｄ坂の殺人事件(056650,order2)→一人二役(057193,order3)のexact
 * work tupleを固定registryとしてmintする。`F008_WORKS`（本ファイル定義）を
 * 別に保持したexpected定数と突き合わせ、tamperを検出する。
 * @des DES-F008-003 @fun FUN-F008-004 @ut UT-F008-004
 */
export function defineF008AuthorAndWorkRegistry(): F008WorkRegistry {
  if (
    F008_WORKS.length !== EXPECTED_REGISTRY_WORKS.length ||
    F008_WORKS.some((work, index) => {
    ...
    new Set(F008_WORKS.map((work) => work.workId)).size !== F008_WORKS.length ||
    AUTHOR_ID !== '001779'
  ) {
    throw new F008SourceError('F008_REGISTRY_MISMATCH', 'F008作者・作品registryが固定値と一致しません');
  }
  const identityCore = {
    authorId: '001779' as const,
    name: 'えどがわらんぽ' as const,
    originalName: '江戸川乱歩' as const,
    slug: 'edogawa-ranpo' as const,
  };
  const identitySha256 = sha256(new TextEncoder().encode(canonicalJson(identityCore)));
  const registry = deepFreeze({
    __brand: 'F008WorkRegistry' as const,
    ...identityCore,
    identitySha256,
    authorMode: 'introduce' as const,
    works: F008_WORKS.map((work) => ({ workId: work.workId, title: work.title, order: work.order })),
  });
...
export function verifyF008AuthorIdentity(
  ...
  if (!isRecord(registry) || !mintedRegistries.has(registry) || registry.__brand !== 'F008WorkRegistry') {
    throw new F008SourceError('F008_REGISTRY_MISMATCH', 'mint済みregistryが必要です');
  }
  if (
    ...
    throw new F008SourceError('F008_REGISTRY_MISMATCH', 'baseline作者・作品との衝突を検出しました');
  }
  const verified = deepFreeze({
    __brand: 'F008VerifiedAuthor' as const,
  ...
  verifiedF008Authors.add(verified);
  return verified;
}

/**
 * @des DES-F008-003 @fun FUN-F008-004
 */
export function isVerifiedF008Author(value: unknown): value is F008VerifiedAuthor {
  return isRecord(value) && verifiedF008Authors.has(value) && value.__brand === 'F008VerifiedAuthor';
}

/**
 * @des DES-F008-004 DES-F008-005 @fun FUN-F008-005 FUN-F008-006
 */
function requireExactF008Context(context: unknown): asserts context is ApprovedBatchContext {
  if (!isMintedApprovedBatchContext(context)) {
    throw new F008SourceError('F008_CONTEXT_INVALID', 'F008のmint済み承認contextが必要です');
  }
  const rawWorks = context.candidate.works;
  if (
    context.definition.feature !== 'F008' ||
    context.definition.batchId !== 'F008' ||
    context.candidate.feature !== 'F008' ||
    context.candidate.author.authorId !== AUTHOR_ID ||
    rawWorks.length !== F008_WORKS.length
  ) {
    throw new F008SourceError('F008_CONTEXT_INVALID', 'F008 contextの固定tupleが一致しません');
  }
  rawWorks.forEach((item, index) => {
    ...
    if (
      ...
      throw new F008SourceError('F008_CONTEXT_INVALID', 'F008 contextの作品順が一致しません');
    }
  });
}
...
function assertTrustedTime(value: Date): string {
  if (!(value instanceof Date) || !Number.isFinite(value.valueOf())) {
    throw new F008SourceError('F008_CONTEXT_INVALID', 'trusted clockが不正です');
  }
 ...
 * VOICEVOX/ずんだもん3規約で共通）をF008向けbrandとして複製する。
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export function evaluateF008PolicyClauses(
  policyId: (typeof F008_POLICY_IDS)[number],
  raw: Uint8Array,
): F008PolicyClauseDecision {
  if (!F008_POLICY_IDS.includes(policyId)) {
    throw new F008SourceError('F008_USAGE_NOT_ALLOWED', '未承認policy IDです');
  }
  const text = policyPlainText(raw);
  const usageForbidden = /(?:無料|非商用|個人利用|広告なし)[^。]{0,80}(?:禁止|認め(?:ない|ません))/u;
  let clauses: F008PolicyClauseResult[];
  ...
  const decision = deepFreeze({
    __brand: 'F008PolicyClauseDecision' as const,
    policyId,
...
function artifact(
  ...
  if (
    ...
    throw new F008SourceError('F008_SOURCE_RESPONSE_INVALID', '公式取得responseが固定条件を満たしません');
  }
  const fetchedAt = response.fetchedAt ?? fallbackFetchedAt;
  if (!Number.isFinite(Date.parse(fetchedAt))) {
    throw new F008SourceError('F008_SOURCE_RESPONSE_INVALID', '公式取得時刻が不正です');
  }
  if (
    !response.security ||
    ...
    throw new F008SourceError('F008_SOURCE_RESPONSE_INVALID', 'transport security evidenceが不正です');
  }
...
function assertExpectedRow(row: BibliographyRow, expected: (typeof F008_WORKS)[number]): void {
  if (
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', `公式書誌が固定条件と一致しません: ${expected.workId}`);
  }
}

const F008_POLICY_ADJUDICATION_PATH = 'content/batches/F008/rights-adjudications.json';
const F008_ADJUDICATION_SHA256 = /^[a-f0-9]{64}$/u;

export interface F008PolicyAdjudication {
...
async function readF008PolicyAdjudications(
  workspace: string,
): Promise<readonly F008PolicyAdjudication[]> {
  let text: string;
  try {
    text = await readFile(resolve(workspace, ...F008_POLICY_ADJUDICATION_PATH.split('/')), 'utf8');
  } catch {
    return [];
  }
  let parsed: unknown;
  try {
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', '裁定記録が不正なJSONです', { cause: error });
  }
  if (canonicalJson(parsed) !== text || !Array.isArray(parsed)) {
    throw new F008SourceError('F008_SOURCE_DRIFT', '裁定記録がcanonical JSON配列ではありません');
  }
  for (const item of parsed) {
    if (!isRecord(item) ||
      !hasExactKeys(item, ['currentSha256', 'decision', 'note', 'policyId', 'previousSha256', 'reviewedAt', 'reviewer']) ||
      !nonBlank(item.policyId) || !F008_ADJUDICATION_SHA256.test(String(item.previousSha256)) ||
      !F008_ADJUDICATION_SHA256.test(String(item.currentSha256)) ||
      ...
      throw new F008SourceError('F008_SOURCE_DRIFT', '裁定記録の項目が不正です');
    }
  }
  return parsed as readonly F008PolicyAdjudication[];
}

/**
 * F008の公式書誌・作家ページ・図書カード・XHTML・VOICEVOX/ずんだもん3規約を
 * productionのHTTPS transportだけで取得し、raw snapshotへ固定する。
 * `f007-source.ts`の`collectF007SourceSnapshot`と同一構造をauthorId `001779`・
 * 人間椅子/Ｄ坂の殺人事件/一人二役へパラメータ化した複製。
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export async function collectF008SourceSnapshot(
  transport: ProductionAozoraTransport,
  context: ApprovedBatchContext,
  phase: F008Phase,
  trustedClock: () => Date,
  options: F008CollectionOptions,
): Promise<F008SourceSnapshot> {
  if (
    !(transport instanceof ProductionAozoraTransport) ||
    transport.request !== ProductionAozoraTransport.prototype.request
  ) {
    throw new F008SourceError('F008_TRANSPORT_REQUIRED', 'production transportが必要です');
  }
  requireExactF008Context(context);
  if (
    !isRecord(options) ||
    ...
    throw new F008SourceError('F008_TRANSPORT_REQUIRED', 'production規約transportとtrusted workspaceが必要です');
  }
  if (phase !== 'selection' && phase !== 'predeploy') {
    throw new F008SourceError('F008_CONTEXT_INVALID', '観測phaseが不正です');
  }
  ...
  try {
    ...
    throw new F008SourceError('F008_SOURCE_RESPONSE_INVALID', '公式書誌ZIPを安全に展開できません', { cause: error });
  }
  ...
  try {
    ...
    throw new F008SourceError('F008_BIBLIOGRAPHY_INVALID', '公式書誌CSVが不正です', { cause: error });
  }
  const selectedRows = F008_WORKS.map((expected) => {
    const matches = rows.filter((row) =>
      row.workId === expected.workId && row.personId === AUTHOR_ID && row.role === '著者',
    );
    if (matches.length !== 1) {
      throw new F008SourceError('F008_SOURCE_DRIFT', `公式書誌の対象行が一意ではありません: ${expected.workId}`);
    }
    const row = matches[0]!;
  ...
  const definitions = createPolicyDefinitions(
    ...
    readonly policyId: (typeof F008_POLICY_IDS)[number];
  } => F008_POLICY_IDS.includes(definition.policyId as (typeof F008_POLICY_IDS)[number]));
  const policies: F008PolicySnapshot[] = [];
  for (const policyId of F008_POLICY_IDS) {
    const definition = definitions.find((item) => item.policyId === policyId);
    if (!definition) {
      throw new F008SourceError('F008_SOURCE_DRIFT', `規約定義がありません: ${policyId}`);
    }
    let fetched;
    try {
      fetched = await fetchPolicyObservation(definition, options.policyTransport);
    } catch (error) {
      throw new F008SourceError('F008_SOURCE_RESPONSE_INVALID', `規約取得に失敗しました: ${policyId}`, { cause: error });
    }
    const bytes = cloneBytes(fetched.body);
    const decision = evaluateF008PolicyClauses(policyId, bytes);
  ...
  const works: F008WorkSnapshot[] = [];
  for (const [index, expected] of F008_WORKS.entries()) {
  ...
  if (phase === 'selection') {
    if (options.selectionSnapshot !== undefined) {
      throw new F008SourceError('F008_CONTEXT_INVALID', 'selection観測へ旧snapshotは指定できません');
    }
  } else {
    const selection = options.selectionSnapshot;
    if (!selection || !mintedSnapshots.has(selection) || selection.phase !== 'selection') {
      throw new F008SourceError('F008_SOURCE_DRIFT', 'predeployにはmint済みselection snapshotが必要です');
    }
    ...
    if (JSON.stringify(worksBefore) !== JSON.stringify(worksAfter)) {
      throw new F008SourceError('F008_SOURCE_DRIFT', 'selection/predeployの原典SHAが変化しました');
    }
    ...
    if (JSON.stringify(rowsBefore) !== JSON.stringify(rowsAfter)) {
      throw new F008SourceError('F008_SOURCE_DRIFT', 'selection/predeployの対象作品の書誌が変化しました');
    }
    ...
    if (drifted.length !== 0) {
      const adjudications = await readF008PolicyAdjudications(options.workspace);
      for (const policy of drifted) {
        ...
        const decided = adjudications.find((item) =>
          ...
          item.decision === 'no-material-change');
        if (!decided) {
          throw new F008SourceError('F008_SOURCE_DRIFT', `規約が変化し裁定記録がありません: ${policy.policyId}`);
        }
      }
    }
  }
 ...
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export function evaluateF008RightsAndUsage(
  snapshot: F008SourceSnapshot,
  usageProfile: F008UsageProfile,
): F008RightsUsageDecision {
  if (!isRecord(snapshot) || !mintedSnapshots.has(snapshot)) {
    throw new F008SourceError('F008_USAGE_NOT_ALLOWED', '検証済みsnapshotが必要です');
  }
  const reasons: string[] = [];
  if (
    snapshot.policies.length !== F008_POLICY_IDS.length ||
    snapshot.policies.some((policy, index) =>
      policy.policyId !== F008_POLICY_IDS[index] ||
  ...
  snapshot.works.forEach((work, index) => {
    try {
      assertExpectedRow(work.bibliography, F008_WORKS[index]!);
    } catch {
 ...
 * `parseBibliographyV2`が持つ夢十夜専用null許可分岐はF008では不要）。
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export function parseF008BibliographyV2(value: unknown, workId: F008WorkId): F008BibliographyV2 {
  if (!F008_WORKS.some((work) => work.workId === workId)) {
    throw new F008SourceError('F008_BIBLIOGRAPHY_INVALID', '未承認work IDです');
  }
  if (
    !isRecord(value) ||
    ...
    throw new F008SourceError('F008_BIBLIOGRAPHY_INVALID', '書誌V2 schemaが不正です（proofreaderは非null必須）');
  }
  return deepFreeze({
...
function extractCardUpdatedAt(card: RawArtifactRef, sourceUrl: string): string {
  if (
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'card raw/SHA bindingが一致しません');
  }
  let text: string;
  try {
    text = new TextDecoder('utf-8', { fatal: true }).decode(card.bytes);
  } catch (error) {
    throw new F008SourceError('F008_SOURCE_DRIFT', 'card UTF-8 decodeに失敗しました', { cause: error });
  }
  const all = [...text.matchAll(/最終更新日/gu)];
  if (all.length !== 1) throw new F008SourceError('F008_SOURCE_DRIFT', 'card最終更新日headerが一意ではありません');
  ...
  if (sourceRows.length > 1) {
    throw new F008SourceError('F008_SOURCE_DRIFT', 'cardの対象XHTML行が一意ではありません');
  }
  ...
  if (!iso) throw new F008SourceError('F008_SOURCE_DRIFT', 'card最終更新日が一意に取得できません');
  const year = iso[1]!;
  ...
  if (
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'card最終更新日が不正です');
  }
  return value;
}

function assertSourceRecordArtifacts(sourceRecord: F008SourceRecordV2): void {
  if (
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'SourceRecordV2 raw/cardが境界前に改変されました');
  }
}
 ...
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export function parseF008SourceRecord(
  snapshot: F008WorkSnapshot,
  expectedWorkId: F008WorkId,
): F008SourceRecordV2 {
  if (!isRecord(snapshot) || !mintedWorkSnapshots.has(snapshot)) {
    throw new F008SourceError('F008_SOURCE_DRIFT', '検証済みwork snapshotが必要です');
  }
  const expected = F008_WORKS.find((work) => work.workId === expectedWorkId);
  if (
    !expected ||
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'source snapshotが固定作品と一致しません');
  }
  const bibliography = parseF008BibliographyV2({
 ...
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export function buildF008SourceProvenance(
  source: F008SourceRecordV2,
  snapshot: F008SourceSnapshot,
...
/**
 * 実HTTPS取得により、人間椅子(056648)・Ｄ坂の殺人事件(056650)・一人二役(057193)の
 * 3作品XHTMLとも未定義named entityは0件（標準5種以外出現なし）であることを実測
 * 確認済みである。F007の舞姫`&nbsp;`のような特別contextはF008には存在しないため、
 * 本関数は常にpassthrough判定のみを許容し、未承認named entityが1件でも
 * 出現した場合はfail-closedで拒否する。
 * @des DES-F008-005 @fun FUN-F008-006 @ut UT-F008-006
 */
export function normalizeF008AozoraXhtmlEntities(
  rawBytes: Uint8Array,
  sourceRecord: F008SourceRecordV2,
): F008EntityNormalizationResult {
  if (!isRecord(sourceRecord) || !mintedSourceRecords.has(sourceRecord)) {
    throw new F008SourceError('F008_ENTITY_NORMALIZATION_INVALID', 'mint済みSourceRecordV2が必要です');
  }
  ...
  if (
    ...
    throw new F008SourceError('F008_ENTITY_NORMALIZATION_INVALID', 'raw source bindingが一致しません');
  }
  let text: string;
  try {
    text = new TextDecoder('shift_jis', { fatal: true }).decode(rawBytes);
  } catch (error) {
    throw new F008SourceError('F008_ENTITY_NORMALIZATION_INVALID', 'Shift_JIS fatal decodeに失敗しました', { cause: error });
  }
...
function preflightF008Xhtml(text: string): void {
  const doctypes = text.match(/<!DOCTYPE\b[\s\S]*?>/giu) ?? [];
  if (doctypes.length !== 1 || canonicalizeDoctype(doctypes[0]!) !== CANONICAL_XHTML11_DOCTYPE) {
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'XHTML 1.1の固定DOCTYPEではありません');
  }
  ...
  if (text.slice(doctypeStart, doctypeEnd + 1).includes('[')) {
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'DOCTYPE internal subsetは禁止です');
  }
  if (
    ...
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', '外部resource/entity構文は禁止です');
  }
  const namedEntities = text.match(/&([A-Za-z][A-Za-z0-9._:-]*);/gu) ?? [];
  if (namedEntities.some((entity) => !['&amp;', '&lt;', '&gt;', '&quot;', '&apos;'].includes(entity))) {
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', '標準5種以外のnamed entityは禁止です');
  }
  let depth = 0;
  let nodes = 0;
  for (const match of text.matchAll(/<(\/?)([A-Za-z][A-Za-z0-9:._-]*)\b[^>]*>/gu)) {
    ...
    if (match[1] === '/') {
      depth -= 1;
      if (depth < 0) throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'XML depthが不正です');
    } else {
      nodes += 1;
      if (!/\/\s*>$/u.test(tag)) depth += 1;
      if (depth > 256 || nodes > 500_000) {
        throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'XML depth/node上限を超えています');
      }
    }
  }
  const textScalars = [...text.replace(/<[^>]*>/gu, '')].length;
  if (text.length > MAX_SOURCE_BYTES || textScalars > 4_000_000 || nodes === 0) {
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'XHTML resource上限を超えています');
  }
}
 ...
 * HTTP 500即時失敗）。REQ-F008-019はこの制約とF007ローカル分割対応（600文字閾値、
 * 句点分割）の再利用を非機能要件として明記しており、F008では設計時点から
 * 本ロジックを組み込む。他フィーチャー（F001〜F007）が依存する共有汎用モジュール
 * （src/voice/generation.ts・src/voice/client.ts・src/content/processing.ts・
 * src/content/batch-production.ts）は変更せず、F008ローカルの抽出層だけで、
 ...
 * @des DES-F008-005 DES-F008-015 @fun FUN-F008-006
 */
const F008_SPLIT_LENGTH_THRESHOLD = 600;

const F008_SPLIT_MEASUREMENT_RULES: SpeechRules = Object.freeze({
...
function groupTokensByThreshold(tokens: readonly TextToken[], threshold: number): TextToken[][] {
  ...
  for (const piece of atomicPieces) {
    ...
    const prospectiveLength = normalizeSpeechText(prospective, F008_SPLIT_MEASUREMENT_RULES).length;
...
function splitOverlongF008Candidates(candidates: readonly RawCandidate[]): RawCandidate[] {
  const expanded = candidates.flatMap((raw) => {
    const speechLength = normalizeSpeechText(raw.tokens, F008_SPLIT_MEASUREMENT_RULES).length;
    if (speechLength <= F008_SPLIT_LENGTH_THRESHOLD) return [raw];
    const groups = groupTokensByThreshold(raw.tokens, F008_SPLIT_LENGTH_THRESHOLD);
 ...
 * @des DES-F008-005 @fun FUN-F008-006 @ut UT-F008-006
 */
export function extractF008DialogueCandidates(
  normalization: F008EntityNormalizationResult,
  source: F008SourceRecordV2,
  extractorVersion: string,
): F008CandidateSet {
  if (!isRecord(source) || !mintedSourceRecords.has(source)) {
    throw new F008SourceError('F008_EXTRACTION_FAILED', 'mint済みSourceRecordV2が必要です');
  }
  ...
  if (
    ...
    throw new F008SourceError('F008_EXTRACTION_FAILED', '抽出input bindingが不正です');
  }
  if (sha256(normalization.processedBytes) !== normalization.processedSha256) {
    throw new F008SourceError('F008_EXTRACTION_FAILED', 'processed artifactのSHAが一致しません');
  }
  let text: string;
  try {
    text = new TextDecoder('shift_jis', { fatal: true }).decode(normalization.processedBytes);
  } catch (error) {
    throw new F008SourceError('F008_XHTML_PREFLIGHT_REJECTED', 'Shift_JIS fatal decodeに失敗しました', { cause: error });
  }
  preflightF008Xhtml(text);
  ...
  if (JSON.stringify(first) !== JSON.stringify(second) || !first.ok) {
    throw new F008SourceError('F008_EXTRACTION_FAILED', '台詞抽出が失敗または非決定的です');
  }
  // 決定性検証(DOM抽出器の素の出力同士の比較)が終わった後だけ分割する。
  // splitOverlongF008Candidates自体は入力に対し決定的なため、この順序でも
  // 「二重実行して一致を確認する」という決定性保証の意味は保たれる。
  const splitCandidates = splitOverlongF008Candidates(first.candidates);
  const resplit = splitOverlongF008Candidates(second.candidates);
  if (JSON.stringify(splitCandidates) !== JSON.stringify(resplit)) {
    throw new F008SourceError('F008_EXTRACTION_FAILED', '長大候補の分割処理が非決定的です');
  }
...
function safePathSegments(relativePosixPath: string): string[] {
  if (
    ...
    throw new F008SourceError('F008_PATH_UNSAFE', 'Windows pathの字句条件に違反しています');
  }
  ...
  if (segments.some((segment) =>
    ...
    throw new F008SourceError('F008_PATH_UNSAFE', 'Windows path segmentが不正です');
  }
  return segments;
}
...
async function safeReadFile(workspace: string, relativePosixPath: string): Promise<Uint8Array> {
  if (!isAbsolute(workspace)) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'workspaceは絶対pathが必要です');
  }
  safePathSegments(relativePosixPath);
  const root = resolve(workspace);
  const rootInfo = await lstat(root);
  if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink() || await realpath(root) !== root) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'workspace実体が不正です');
  }
  const target = join(root, ...relativePosixPath.split('/'));
  const relation = relative(root, target);
  if (!relation || relation === '..' || relation.startsWith(`..${sep}`) || isAbsolute(relation)) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'source pathがworkspace外です');
  }
  let cursor = root;
  for (const component of relation.split(sep)) {
    cursor = join(cursor, component);
    const info = await lstat(cursor);
    if (info.isSymbolicLink()) throw new F008SourceError('F008_PATH_UNSAFE', 'source pathにreparseがあります');
  }
  const info = await lstat(target);
  if (!info.isFile() || info.isSymbolicLink() || await realpath(target) !== target) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'source実体が通常fileではありません');
  }
...
async function atomicWriteFile(workspace: string, relativePosixPath: string, bytes: Uint8Array): Promise<void> {
  ...
  try {
    ...
    throw new F008SourceError('F008_PATH_UNSAFE', `既存artifactが不一致です: ${relativePosixPath}`);
  } catch (error) {
    if (error instanceof F008SourceError) throw error;
  }
...
function selectionDataDirectory(): string {
  return 'data/batches/F008/source-snapshots/selection';
}

function predeployDataPath(run: string, leaf: string): string {
  return `data/batches/F008/source-snapshots/selection/predeploy-${run}-${leaf.replaceAll('/', '-')}`;
}

/**
 * 取得済みF008 snapshotを`content/batches/F008/source-snapshots/`配下の
 * canonical JSONと、raw byte実体（`data/batches/F008/source-snapshots/...`）へ
 * atomicに固定する。CSVはZIPから再導出可能なため`storage: 'derived'`として
 * 別ファイルを持たない（`f005-source.ts`の`rehydrateDerivedArtifact`と同じ最適化）。
 * @des DES-F008-004 DES-F008-005 @fun FUN-F008-005 FUN-F008-006
 */
export async function persistF008SourceSnapshot(
  workspace: string,
  snapshot: F008SourceSnapshot,
  run?: string,
): Promise<WorkspaceRelativePath> {
  if (!isRecord(snapshot) || !mintedSnapshots.has(snapshot)) {
    throw new F008SourceError('F008_SOURCE_DRIFT', '検証済みsnapshotが必要です');
  }
  if (snapshot.phase === 'predeploy' && !nonBlank(run)) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'predeploy永続化にはrun識別子が必要です');
  }
  ...
  const targetRelative = snapshot.phase === 'selection'
    ? 'content/batches/F008/source-snapshots/selection.json'
    : `content/batches/F008/source-snapshots/predeploy-${run}.json`;
...
async function rehydrateArtifact(
  ...
  if (!isRecord(value) || !hasExactKeys(value, [
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'persisted artifact metadataが固定tupleと一致しません');
  }
  const bytes = await safeReadFile(workspace, expected.path);
  if (bytes.byteLength !== value.byteLength || sha256(bytes) !== value.sha256) {
    throw new F008SourceError('F008_SOURCE_DRIFT', 'persisted artifact実体のSHAまたはbyte数が一致しません');
  }
  return deepFreeze({
    storage: 'inline' as const,
    sourceUrl: expected.sourceUrl,
...
async function rehydrateSnapshotDocument(
  ...
  try {
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'snapshot JSONが不正です', { cause: error });
  }
  if (documentText !== canonicalJson(persisted) || !isRecord(persisted) ||
    ...
    persisted.policies.length !== F008_POLICY_IDS.length ||
    !Array.isArray(persisted.works) ||
    persisted.works.length !== F008_WORKS.length
  ) {
    throw new F008SourceError('F008_SOURCE_DRIFT', 'snapshotのcanonical schemaが一致しません');
  }
  ...
  try {
    ...
    throw new F008SourceError('F008_BIBLIOGRAPHY_INVALID', 'persisted公式書誌を再検証できません', { cause: error });
  }
  ...
  if (!isRecord(rawCsvMeta) || !hasExactKeys(rawCsvMeta, [
    ...
    throw new F008SourceError('F008_SOURCE_DRIFT', 'persisted ZIP/CSV bindingが一致しません');
  }
  const bibliographyCsv = deepFreeze({
    storage: 'inline' as const,
    sourceUrl: AOZORA_BIBLIOGRAPHY_URL,
  ...
  try {
    ...
    throw new F008SourceError('F008_BIBLIOGRAPHY_INVALID', 'persisted公式書誌CSVを解析できません', { cause: error });
  }
  const selectedRows = F008_WORKS.map((expected) => {
    const matches = rows.filter((row) =>
      row.workId === expected.workId && row.personId === AUTHOR_ID && row.role === '著者');
    if (matches.length !== 1) {
      throw new F008SourceError('F008_SOURCE_DRIFT', `persisted書誌の対象行が一意ではありません: ${expected.workId}`);
    }
  ...
  const definitions = createPolicyDefinitions(workspace, workspace, 'F008')
    .filter((definition): definition is typeof definition & {
      readonly policyId: (typeof F008_POLICY_IDS)[number];
    } => F008_POLICY_IDS.includes(definition.policyId as (typeof F008_POLICY_IDS)[number]));
  const policies: F008PolicySnapshot[] = [];
  for (const [index, policyId] of F008_POLICY_IDS.entries()) {
    ...
    if (!definition || !isRecord(rawPolicy) || !hasExactKeys(rawPolicy, ['policyId', 'versionOrLabel', 'artifact', 'decision']) ||
      ...
      throw new F008SourceError('F008_SOURCE_DRIFT', `persisted規約tupleが一致しません: ${policyId}`);
    }
    ...
    const decision = evaluateF008PolicyClauses(policyId, policyArtifact.bytes);
    if (canonicalJson(rawPolicy.decision) !== canonicalJson(decision)) {
      throw new F008SourceError('F008_SOURCE_DRIFT', `persisted規約decisionが本文と一致しません: ${policyId}`);
    }
  ...
  for (const [index, expected] of F008_WORKS.entries()) {
    ...
    if (!isRecord(rawWork) || !hasExactKeys(rawWork, ['workId', 'title', 'bibliography', 'card', 'xhtml']) ||
      ...
      throw new F008SourceError('F008_SOURCE_DRIFT', `persisted作品とapproved contextが一致しません: ${expected.workId}`);
    }
  ...
  if (rights.decision !== 'allow' || canonicalJson(persisted.rights) !== canonicalJson(rights)) {
    throw new F008SourceError('F008_USAGE_NOT_ALLOWED', 'persisted rights decisionを再現できません');
  }
  return snapshot;
}
 ...
 * @des DES-F008-004 @fun FUN-F008-005 @ut UT-F008-005
 */
export async function rehydrateF008SelectionSnapshot(
  workspace: string,
  context: ApprovedBatchContext,
): Promise<F008SourceSnapshot> {
...
const F008_PREDEPLOY_SNAPSHOT_PATH_PATTERN =
  /^content\/batches\/F008\/source-snapshots\/predeploy-([0-9A-Za-z-]+)\.json$/u;

/**
 * process再起動後にpredeploy runを実ファイル・承認contextへ再結合して再mintする。
 * @des DES-F008-004 DES-F008-005 @fun FUN-F008-005 FUN-F008-006 @ut UT-F008-006
 */
export async function rehydrateF008PredeploySnapshot(
  workspace: string,
  context: ApprovedBatchContext,
  snapshotRelativePath: string,
): Promise<F008SourceSnapshot> {
  const match = typeof snapshotRelativePath === 'string'
    ? F008_PREDEPLOY_SNAPSHOT_PATH_PATTERN.exec(snapshotRelativePath)
    : null;
  if (!match) {
    throw new F008SourceError('F008_PATH_UNSAFE', 'predeploy snapshot pathが固定形式ではありません');
  }
  const run = match[1]!;
