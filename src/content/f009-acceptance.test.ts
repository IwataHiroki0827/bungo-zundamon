...
import {
  acceptF009Work,
  F009AcceptanceError,
  prepareF009WorkAcceptance,
  recoverF009WorkAcceptance,
} from './f009-acceptance.ts';

/**
 * UT-F009-010: F009 work単位atomic受入。src/content/f008-acceptance.test.tsの
 * F009向けパラメータ化複製（skip()付きfixtureパターンを踏襲）。
 *
 * F009の3作品（瓶詰地獄002381・きのこ会議046694・死後の恋002380）は
 * 全てaccepted済みである。assertOrderは対象workIdより後続のworkがaccepted
 * であることを許さないため、manifest末尾（最後にaccept遷移した）work IDの
 * みが再検証対象になり得る（F006/F007/F008と同型の制約）。
 *
 * mint保護・work順・path allowlist・workId形式の4ケースは、accepted work
 * artifactを必要とせず実workspace（content/batches/F009/batch.json、
 * 3作品ともmanifestに存在しworkIds順を持つ）だけで実行可能なため、
 * mock無しで実際に検証する。
 * @des DES-F009-009 @fun FUN-F009-010 @ut UT-F009-010
 */

const workspace = resolve(process.cwd());
const MANIFEST_PATH = 'content/batches/F009/batch.json';
// F009の3作品は全てaccepted済みのため、work順違反(F009_WORK_ORDER)なく
// 再検証できるのはmanifest末尾（最後にaccept遷移した）work IDのみ。
const TARGET_WORK_ID = '002380' as WorkId;
// manifestに存在しないwork IDでのwork順違反（F009_WORK_ORDER）は、
...
/**
 * `.cache/batch-accept/F009/{id}`配下4ファイル（gitignore対象・非committed）と
 * `.cache/transactions/accepted-audio/F009-{id}.json`（journal、同じくgitignore
 * 対象）を、実workspaceのcanonical artifactから導出できる値だけを用いて
 * 一時workspace内へ新規構築する。src/content/f008-acceptance.test.tsの
...
async function buildCacheFixtures(root: string): Promise<void> {
  ...
  const actualPath = `content/batches/F009/capacity-actual/${id}.json`;
  ...
  const fixtureBytes = new TextEncoder().encode('F009 acceptance fixture\n');
  ...
  await writeJson(root, `.cache/batch-accept/F009/${id}/content-preview.json`, {
    activeBatchId: 'F009',
    activeWorkId: id,
    buildSha256: digest,
    files: [fixtureFile],
  ...
  await writeJson(root, `.cache/batch-accept/F009/${id}/dist-preview.json`, {
    batchId: 'F009',
    contentBuildSha256: digest,
    distSha256,
    workId: id,
  });
  await writeJson(root, `.cache/batch-accept/F009/${id}/f001-content-invariant.json`, {
    baselineSha256: 'a'.repeat(64),
    buildSha256: digest,
    result: 'pass',
    stagingSha256: digest,
  });
  await writeJson(root, `.cache/batch-accept/F009/${id}/f001-dist-invariant.json`, {
    contentBuildSha256: digest,
    distSha256,
    result: 'pass',
  });
  ...
  // 新buildSha256へ置換する（prepareF009WorkAcceptanceが
  ...
  // journal（`.cache/transactions/accepted-audio/F009-{id}.json`）。既にaccepted済み
  // workのみ再現可能（f008版と同型の既知パターン）。
  const sourceJournalPath = resolve(workspace, '.cache', 'transactions', 'accepted-audio', `F009-${id}.json`);
  ...
  await writeJson(root, `.cache/transactions/accepted-audio/F009-${id}.json`, {
    batchId: 'F009',
    evidence,
    ...
    phase: 'verified',
    schemaVersion: '1.0.0',
    staging: `content/batches/F009/.accepted-audio-staging-${randomUUID()}-${id}`,
    target: `content/batches/F009/accepted-audio/${id}`,
    workId: id,
  });
}

/**
 * 一時workspaceへ、prepareF009WorkAcceptance/acceptF009Workが読む
...
async function fixture(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'f009-acceptance-'));
  const id = TARGET_WORK_ID;
  const paths = [
    MANIFEST_PATH,
    `content/batches/F009/voice-evidence/${id}.json`,
    `content/batches/F009/work-artifacts/${id}/voice-generation.json`,
    `content/batches/F009/work-artifacts/${id}/voice-completeness.json`,
    `content/batches/F009/capacity-actual/${id}.json`,
  ] as const;
  ...
  await cp(
    join(workspace, 'content', 'batches', 'F009', 'accepted-audio'),
    join(root, 'content', 'batches', 'F009', 'accepted-audio'),
    { recursive: true },
  ).catch(() => undefined);
...
describe('f009-acceptance', () => {
  it('永続化済みcanonical artifactからprepareし、accepted workをbrandedに再検証できる', async ({ skip }) => {
    ...
    const prepared = await prepareF009WorkAcceptance(root, MANIFEST_PATH, TARGET_WORK_ID);
    expect(prepared.batchId).toBe('F009');
    ...
    const result = await acceptF009Work(root, prepared, expectedManifestSha);
    ...
    const recovered = await recoverF009WorkAcceptance(root, prepared, expectedManifestSha);
  ...
  it('mintされていないprepared値では受入を拒否する', async () => {
    ...
    const expectedManifestSha = hashBatchManifest(checked.value);
    const forged = {
      __brand: 'PreparedF009WorkAcceptance',
      batchId: 'F009',
      workId: TARGET_WORK_ID,
      manifestPath: MANIFEST_PATH,
      expectedManifestSha,
    } as unknown as Parameters<typeof acceptF009Work>[1];
    await expect(acceptF009Work(workspace, forged, expectedManifestSha)).rejects.toThrow(F009AcceptanceError);
  });

  it('manifestに存在しないwork IDはwork順違反として拒否する', async () => {
    await expect(prepareF009WorkAcceptance(workspace, MANIFEST_PATH, UNKNOWN_WORK_ID))
      .rejects.toMatchObject({ code: 'F009_WORK_ORDER' });
  });

  it('manifest pathがallowlist外なら拒否する', async () => {
    await expect(prepareF009WorkAcceptance(workspace, 'content/batches/F002/batch.json', TARGET_WORK_ID))
      .rejects.toMatchObject({ code: 'F009_ACCEPTANCE_PATH' });
  });

  it('workId形式が不正なら拒否する', async () => {
    await expect(prepareF009WorkAcceptance(workspace, MANIFEST_PATH, 'not-a-work-id'))
      .rejects.toMatchObject({ code: 'F009_ACCEPTANCE_PATH' });
  });
});
