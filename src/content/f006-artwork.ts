...
/**
 * F006（中島敦3作品追加）専用の作者画像provenance軽量schema。
 *
 * DD-F006.md FUN-F006-014/015の方式訂正(2026-08-22)により、F001〜F005共有の
 * OpenAI内蔵image_genツール専用検証（`src/notices/artwork-provenance.ts`）は
 * 一切変更・再利用しない。F006の作者画像はローカルComfyUI（FLUX.1 schnell）で
 * 生成しており、`provider === 'OpenAI'`固定検証を通せないため。F005型の
 * `F005ApprovedBatchContext`三段階承認controlも新設しない（DES-F006-001の
 ...
 *
 * @des DES-F006-012 @fun FUN-F006-014 FUN-F006-015
 */
...
export type F006ArtworkErrorCode =
  | 'F006_ARTWORK_PROVENANCE_INVALID'
  | 'F006_ARTWORK_BINDING_INVALID'
  | 'F006_ARTWORK_EXISTING_INVALID'
  | 'F006_ARTWORK_NEAR_DUPLICATE';

export class F006ArtworkError extends Error {
  constructor(
    public readonly code: F006ArtworkErrorCode,
    message: string,
    options?: ErrorOptions,
  ) {
    super(message, options);
    this.name = 'F006ArtworkError';
  }
}

export interface F006ArtworkManifestIdentity {
  readonly batchId: 'F006';
  readonly authorId: '000119';
  readonly outputPath: 'artwork/nakajima-zundamon.png';
}

export interface ArtworkGenerationF006Input {
...
export interface ArtworkInputsF006 {
  readonly referenceInputs: readonly [];
}

export interface F006FinalArtworkInput {
  readonly sourcePath: 'content/batches/F006/public-files/artwork/nakajima-zundamon.png';
  readonly publicPath: 'artwork/nakajima-zundamon.png';
  readonly credit: string;
  readonly bytes: Uint8Array;
}

export interface ArtworkProvenanceF006 {
  readonly schemaVersion: '1.0.0';
  readonly manifestId: 'artwork-F006-000119-v1';
  readonly identity: F006ArtworkManifestIdentity;
  ...
  readonly output: Readonly<{
    readonly sourcePath: F006FinalArtworkInput['sourcePath'];
    readonly publicPath: F006FinalArtworkInput['publicPath'];
...
export interface ExistingArtworkInputF006 {
...
export interface ArtworkAcceptanceF006 {
  readonly result: 'pass';
  readonly path: F006FinalArtworkInput['publicPath'];
  readonly alt: '中島敦をイメージしたずんだもんの独自イラスト';
  readonly sha256: Sha256;
  readonly dHash64: DHash64;
  readonly sourceProvenancePath: 'content/batches/F006/artwork-provenance.json';
  readonly provenanceSha256: Sha256;
  readonly credit: string;
...
function artworkError(
  code: F006ArtworkErrorCode,
  message: string,
  options?: ErrorOptions,
): never {
  throw new F006ArtworkError(code, message, options);
}
...
/** F005実装済みのpngjs decoderを経由してdHash64を求める。decode失敗はF006側codeへ変換する。 */
function safeComputeDHash64(bytes: Uint8Array): DHash64 {
  try {
    return computeDHash64V1(bytes);
  } catch (error) {
    if (error instanceof F005ArtworkError) {
      return artworkError('F006_ARTWORK_PROVENANCE_INVALID', `PNG decodeに失敗しました: ${error.message}`, { cause: error });
    }
    throw error;
  }
}
...
function provenancePayload(
  value: Omit<ArtworkProvenanceF006, 'provenanceSha256'>,
): Omit<ArtworkProvenanceF006, 'provenanceSha256'> {
  return value;
}

/**
 * exact final image path・参照入力exact []・実際の生成情報のみを記録した
 * ArtworkProvenanceF006をsealする。捏造した値は一切書かず、`generation`引数の
 * 実値だけをcanonical化する。
 * @des DES-F006-012 @fun FUN-F006-014 @ut UT-F006-014
 */
export function sealF006ArtworkProvenance(
  generation: ArtworkGenerationF006Input,
  inputs: ArtworkInputsF006,
  finalImage: F006FinalArtworkInput,
  ...
  if (
    ...
    return artworkError('F006_ARTWORK_PROVENANCE_INVALID', 'generation schemaが不正です');
  }
  if (
    !isRecord(inputs) || !exactKeys(inputs, ['referenceInputs']) ||
    !Array.isArray(inputs.referenceInputs) || inputs.referenceInputs.length !== 0
  ) {
    return artworkError('F006_ARTWORK_PROVENANCE_INVALID', 'referenceInputsはexact []が必要です');
  }
  if (
    !isRecord(finalImage) ||
    !exactKeys(finalImage, ['bytes', 'credit', 'publicPath', 'sourcePath']) ||
    finalImage.sourcePath !==
      'content/batches/F006/public-files/artwork/nakajima-zundamon.png' ||
    finalImage.publicPath !== 'artwork/nakajima-zundamon.png' ||
    !nonBlank(finalImage.credit) ||
    /[<>]/u.test(finalImage.credit) ||
    !(finalImage.bytes instanceof Uint8Array)
  ) {
    return artworkError('F006_ARTWORK_PROVENANCE_INVALID', 'final image schema/path/creditが不正です');
  }
...
export function serializeF006ArtworkProvenance(
  provenance: ArtworkProvenanceF006,
): string {
  if (!sealedProvenances.has(provenance) && !rehydratedProvenances.has(provenance)) {
    return artworkError('F006_ARTWORK_BINDING_INVALID', 'mintされていないprovenanceです');
  }
 ...
 */
export function parseAndRehydrateF006ArtworkProvenance(
  ...
  if (!isRecord(parsed) || canonicalJson(parsed) !== canonicalArtifact) {
    return artworkError('F006_ARTWORK_PROVENANCE_INVALID', 'provenance artifactがcanonical JSONではありません');
  }
  const expected = sealF006ArtworkProvenance(generation, inputs, finalImage);
  ...
  const restored = freezeDeep(parsed as unknown as ArtworkProvenanceF006);
  rehydratedProvenances.add(restored);
  return restored;
}

export function isMintedF006ArtworkProvenance(
  value: unknown,
): value is ArtworkProvenanceF006 {
  return isRecord(value) && (sealedProvenances.has(value) || rehydratedProvenances.has(value));
}

/**
 * 既存4作者画像を実体から再計算し、byte一致とdHash距離8以下を
 * `F006_ARTWORK_NEAR_DUPLICATE`として拒否する。authorId `000119`が既存4作者と
 * 衝突しないことも検証する。
 * @des DES-F006-012 @fun FUN-F006-015 @ut UT-F006-015
 */
export function verifyF006ArtworkAgainstCatalog(
  provenance: ArtworkProvenanceF006,
  finalImage: Uint8Array,
  existingArtwork: readonly ExistingArtworkInputF006[],
): ArtworkAcceptanceF006 {
  if (
    ...
    return artworkError('F006_ARTWORK_BINDING_INVALID', 'provenance/final image bindingが不正です');
  }
  // F006時点は既存4作者（F001・F002・F003・F005）固定を想定していたが、F007以降が
  // 新規authorを追記し続けるappend-only catalog設計のため、以降は「最低4作者以上」へ
  // 緩和する(F005-artwork.tsの2026-08-22修正と同型。近似重複検出はexistingArtwork
  // 全件に対して行われるため、件数が増えても検証の厳密さは損なわれない)。
  if (!Array.isArray(existingArtwork) || existingArtwork.length < 4) {
    return artworkError('F006_ARTWORK_EXISTING_INVALID', '既存4作者以上の画像が必要です');
  }
  ...
  const comparisons = existingArtwork.map((existing) => {
    ...
    if (
      ...
      !nonBlank(authorId) || authorId === '000119' ||
      ...
      return artworkError('F006_ARTWORK_EXISTING_INVALID', '既存画像identity/path/schemaが不正です');
    }
    ...
    if (actualSha256 !== claimedSha256 || actualDHash !== existing.dHash64) {
      return artworkError('F006_ARTWORK_EXISTING_INVALID', '既存画像の自己申告hashが実体と一致しません');
    }
  ...
  const acceptance = freezeDeep({
    ...
    alt: '中島敦をイメージしたずんだもんの独自イラスト' as const,
    sha256: provenance.output.sha256,
    dHash64: provenance.output.dHash64,
    sourceProvenancePath: 'content/batches/F006/artwork-provenance.json' as const,
...
export function isMintedF006ArtworkAcceptance(
  value: unknown,
): value is ArtworkAcceptanceF006 {
