...
/**
 * F007（中島敦3作品追加）work単位の差分音声生成・容量forecast thin script。
 * DD-F007.md FUN-F007-009記載のとおり、既存の汎用モジュール
 ...
 * を直接呼び出すだけで、F007専用の新規wrapperモジュールは作らない。
 *
 ...
 * content/batches/F007/work-artifacts/<workId>/.voice-stage-<workId>/ に保持する。
 *
 * T-152（名人伝）着手時にwork ID固定を解消しCLI引数化した（T-153弟子でも再利用するため）。
 * @des DES-F007-008 @fun FUN-F007-009
 */
...
const FORECAST_TOOL_VERSION = 'f007-prepare-voice-forecast-v1';
const VOICE_TOOL_VERSION = 'f007-prepare-voice-generate-v1';
...
/**
 * f007-prepare-editorial.tsのadvanceF007WorkStateと同じ再開可能パターンを
 * budget-approved/voicedへ拡張する。同一入力での再実行はstageRecordsの
 * 既存出力と照合し、一致すれば何もしない。
 */
async function advanceF007VoiceWorkState(
  workspace: string,
...
async function main(): Promise<void> {
  ...
  if (revisionArtifact.value.batchId !== BATCH_ID || revisionArtifact.value.workId !== WORK_ID || revisionArtifact.value.speech.length === 0) {
    throw new Error(`speech-revisions.jsonがF007/${WORK_ID}のtupleと一致しません`);
  }
  ...
  if (workProgress.status === 'reviewed') {
    ...
    if (forecast.result === 'blocked' || !forecast.canGenerate) {
      throw new Error(`F007 capacity forecast blocked: ${forecast.reasons.join(',')}`);
    }
    ...
    manifest = await advanceF007VoiceWorkState(
      workspace,
      manifest,
      'budget-approved',
      [forecastSha],
  ...
  if (generation.failed !== 0 || generation.succeeded !== authorizedPlan.uniqueAudioCount) {
    throw new Error('F007 voice generationが不完全です');
  }
