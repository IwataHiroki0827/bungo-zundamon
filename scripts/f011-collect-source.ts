...
/**
 * F011（新美南吉3作品追加）のselection段階原典・書誌・規約を実HTTPS取得し、
 * content/batches/F011/source-snapshots/selection.json + data/batches/F011/
 * source-snapshots/selection/配下へ永続化する。f009-collect-source.tsと
 * 同じ構成（native guard/ETW不使用）。
 * @des DES-F011-004 @fun FUN-F011-005
 */
async function main(): Promise<void> {
  ...
  if (rights.decision !== 'allow') {
    throw new Error(`F011 rights/usageがblockedです: ${rights.reasons.join(',')}`);
  }
  const targetPath = await persistF011SourceSnapshot(workspace, snapshot);
