/**
 * F005/F006/F007/F008/F009と同型の不備（CHG-F005-078・F006/F007/F008/F009側是正参照）が
 * F010にも再発した: F010 batch.jsonはbatch level `stageRecords`が空のまま（F002〜F004が
 * 経由する共有`transitionBatchState('rights-verified', ...)`を一度も通らない設計）である
 * ため、`rightsSnapshotIds`が空配列のまま3作品全件acceptedへ進んでしまっていた。
 ...
 * selection.jsonの`kind`フィールドは実データ上`f010-source-selection-snapshot`
 * （F010向けgenerator実装時に正しくリネームされている）であり、本scriptはこの
 * 実測値をそのまま検証条件とする。
 *
...
const MANIFEST_PATH = join(WORKSPACE, 'content', 'batches', 'F010', 'batch.json');
const SELECTION_PATH = join(WORKSPACE, 'content', 'batches', 'F010', 'source-snapshots', 'selection.json');
...
async function deriveRightsSnapshotIds(): Promise<readonly string[]> {
  ...
  if (record.kind !== 'f010-source-selection-snapshot' || record.batchId !== 'F010' ||
...
async function main(): Promise<void> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F010' || canonicalJson(checked.value) !== manifestText) {
    throw new Error(`既存batch.jsonがcanonical F010 manifestではありません: ${!checked.ok ? checked.error.code : 'text mismatch'}`);
  }
