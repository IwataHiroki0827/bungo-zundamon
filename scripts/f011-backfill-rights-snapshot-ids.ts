/**
 * F005/F006/F007/F008/F009/F010と同型の不備（CHG-F005-078・F006/F007/F008/F009/F010側是正
 * 参照）がF011にも再発した: F011 batch.jsonはbatch level `stageRecords`が空のまま
 * （F002〜F004が経由する共有`transitionBatchState('rights-verified', ...)`を一度も通らない
 * 設計）であるため、`rightsSnapshotIds`が空配列のまま3作品全件acceptedへ進んでしまって
 * いた。work状態遷移はforward-onlyのため、通常のacceptedフローをやり直すことはできない。
 ...
 * selection.jsonの`kind`フィールドは実データ上`f011-source-selection-snapshot`
 * （F011向けgenerator実装時に正しくリネームされている）であり、本scriptはこの
 * 実測値をそのまま検証条件とする。
 *
...
const MANIFEST_PATH = join(WORKSPACE, 'content', 'batches', 'F011', 'batch.json');
const SELECTION_PATH = join(WORKSPACE, 'content', 'batches', 'F011', 'source-snapshots', 'selection.json');
...
async function deriveRightsSnapshotIds(): Promise<readonly string[]> {
  ...
  if (record.kind !== 'f011-source-selection-snapshot' || record.batchId !== 'F011' ||
...
async function main(): Promise<void> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F011' || canonicalJson(checked.value) !== manifestText) {
    throw new Error(`既存batch.jsonがcanonical F011 manifestではありません: ${!checked.ok ? checked.error.code : 'text mismatch'}`);
  }
