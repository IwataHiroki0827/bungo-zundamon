...
/**
 * F009（夢野久作3作品追加）work単位の容量actual測定＋atomic受入前橋渡しscript。
 * scripts/f008-capacity-actual.tsのF009向けパラメータ化複製。F008と同じ理由で、
 * F009のvoiced stage記録も間接ハッシュ規約（voiceEvidenceRefファイルのSHA-256だけを
 * 保持、scripts/f009-prepare-voice.ts）を採用しており、汎用promoteVerifiedWorkArtifacts
 * のassertInputsと直接には噛み合わないため、manifestを直接操作して直接ハッシュ値を持つ
 * voiced stage recordとcapacity-actual stage recordを追記する。
 * @des DES-F009-008 DES-F009-009 @fun FUN-F009-009 FUN-F009-010
 */

const BATCH_ID = 'F009';
const workIdArgument = process.argv[2];
if (!workIdArgument || !/^[0-9]{6}$/u.test(workIdArgument)) {
  throw new Error('6桁のwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f009-capacity-actual.ts 002381）');
}
...
const VOICED2_TOOL_VERSION = 'f009-capacity-actual-voiced-direct-v1';
const ACTUAL_TOOL_VERSION = 'f009-capacity-actual-v1';
...
async function loadManifest(workspace: string): Promise<BatchManifest> {
  ...
  if (!checked.ok || canonicalJson(checked.value) !== text || checked.value.batchId !== BATCH_ID) {
    throw new Error(`F009 manifestが不正です: ${checked.ok ? 'not canonical' : checked.error.code}`);
  }
...
async function main(): Promise<void> {
  ...
  if (preview.value.mode !== 'work-preview' || preview.value.activeBatchId !== BATCH_ID || preview.value.activeWorkId !== WORK_ID) {
    throw new Error('content-preview.jsonが現在のwork-previewと一致しません（scripts/f009-content-preview.tsを先に実行してください）');
  }
  ...
  try {
    ...
    if (actual.result === 'blocked') throw new Error(`F009容量actualがblockedです: ${actual.reasons.join(',')}`);
