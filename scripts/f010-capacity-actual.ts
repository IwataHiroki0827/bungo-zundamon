...
/**
 * F010（梶井基次郎3作品追加）work単位の容量actual測定＋atomic受入前橋渡しscript。
 * scripts/f009-capacity-actual.tsのF010向けパラメータ化複製。F009と同じ理由で、
 * F010のvoiced stage記録も間接ハッシュ規約（voiceEvidenceRefファイルのSHA-256だけを
 * 保持、scripts/f010-prepare-voice.ts）を採用しており、汎用promoteVerifiedWorkArtifacts
 * のassertInputsと直接には噛み合わないため、manifestを直接操作して直接ハッシュ値を持つ
 * voiced stage recordとcapacity-actual stage recordを追記する。
 * @des DES-F010-008 DES-F010-009 @fun FUN-F010-009 FUN-F010-010
 */

const BATCH_ID = 'F010';
const workIdArgument = process.argv[2];
if (!workIdArgument || !/^[0-9]{6}$/u.test(workIdArgument)) {
  throw new Error('6桁のwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f010-capacity-actual.ts 000424）');
}
...
const VOICED2_TOOL_VERSION = 'f010-capacity-actual-voiced-direct-v1';
const ACTUAL_TOOL_VERSION = 'f010-capacity-actual-v1';
...
async function loadManifest(workspace: string): Promise<BatchManifest> {
  ...
  if (!checked.ok || canonicalJson(checked.value) !== text || checked.value.batchId !== BATCH_ID) {
    throw new Error(`F010 manifestが不正です: ${checked.ok ? 'not canonical' : checked.error.code}`);
  }
...
async function main(): Promise<void> {
  ...
  const [voiceEvidence, generationCanonical, completeness, forecastArtifact] = await Promise.all([
    canonicalArtifact<{
      ...
      readonly kind: 'f009-voice-evidence';
  ...
  if (preview.value.mode !== 'work-preview' || preview.value.activeBatchId !== BATCH_ID || preview.value.activeWorkId !== WORK_ID) {
    throw new Error('content-preview.jsonが現在のwork-previewと一致しません（scripts/f010-content-preview.tsを先に実行してください）');
  }
  ...
  const pagesOutputRoot = join(workspace, '.cache', `.f009-actual-pages-${WORK_ID}-${Date.now()}`);
  ...
  try {
    ...
    if (actual.result === 'blocked') throw new Error(`F010容量actualがblockedです: ${actual.reasons.join(',')}`);
