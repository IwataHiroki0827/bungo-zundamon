...
/**
 * F011（新美南吉3作品追加）work単位の差分音声生成・容量forecast thin script。
 * f010-prepare-voice.tsをF011向けにパラメータ化した複製。CHG-F008-001是正済み
 * 固定上限方式（`Math.min(currentPages.bytes, 150_000_000)`）をF011でも踏襲する。
 * @des DES-F011-008 @fun FUN-F011-009
 */

const BATCH_ID = 'F011';
const workIdArgument = process.argv[2];
if (!workIdArgument || !/^[0-9]{6}$/u.test(workIdArgument)) {
  throw new Error('6桁のwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f011-prepare-voice.ts 000637）');
}
...
const FORECAST_TOOL_VERSION = 'f011-prepare-voice-forecast-v1';
const VOICE_TOOL_VERSION = 'f011-prepare-voice-generate-v1';
...
/**
 * f011-prepare-editorial.tsのadvanceF011WorkStateと同じ再開可能パターンを
 * budget-approved/voicedへ拡張する。同一入力での再実行はstageRecordsの
 * 既存出力と照合し、一致すれば何もしない。
 */
async function advanceF011VoiceWorkState(
  workspace: string,
...
async function main(): Promise<void> {
  ...
  if (revisionArtifact.value.batchId !== BATCH_ID || revisionArtifact.value.workId !== WORK_ID || revisionArtifact.value.speech.length === 0) {
    throw new Error(`speech-revisions.jsonがF011/${WORK_ID}のtupleと一致しません`);
  }
  ...
  if (workProgress.status === 'reviewed') {
    ...
    if (forecast.result === 'blocked' || !forecast.canGenerate) {
      throw new Error(`F011 capacity forecast blocked: ${forecast.reasons.join(',')}`);
    }
    const forecastArtifact = {
      schemaVersion: '1.0.0' as const,
      kind: 'f011-voice-capacity-forecast' as const,
      batchId: BATCH_ID,
      workId: WORK_ID,
      expectedManifestSha: binding.expectedManifestSha,
      preTreeDigest: binding.preTreeDigest,
    ...
    manifest = await advanceF011VoiceWorkState(
      workspace,
      manifest,
      'budget-approved',
      [forecastSha],
  ...
  if (generation.failed !== 0 || generation.succeeded !== authorizedPlan.uniqueAudioCount) {
    throw new Error('F011 voice generationが不完全です');
  }
  ...
  const voiceEvidence = {
    schemaVersion: '1.0.0' as const,
    kind: 'f011-voice-evidence' as const,
    batchId: BATCH_ID,
    workId: WORK_ID,
