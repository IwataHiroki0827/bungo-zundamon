/**
 * F005と同型の不備（CHG-F005-078参照）がF006にも再発した: F006 batch.jsonは
 * `content/batches/F006/batch.json`のbatch level `stageRecords`が空のまま
 * （F002〜F004が経由する共有`transitionBatchState('rights-verified', ...)`を
 * 一度も通らない設計）であるため、`rightsSnapshotIds`が空配列のまま3作品全件
 * acceptedへ進んでしまっていた（`work.actualCapacityRef`は
 * `src/content/batch-acceptance.ts`の`promoteVerifiedWorkArtifacts`が
 * work単位で正しく書き込み済みで欠落なし）。
 * work状態遷移はforward-onlyのため、通常のacceptedフローをやり直すことはできない。
 * このスクリプトは、実在するevidence（selection.jsonのpolicies[].decision.contentSha256、
 * 選定時2026-08-21に取得・検証済み）から機械的に値を導出し、batch.jsonへ
 * 一度だけ直接書き込む。値は捏造せず、必ず実ファイルを読んで検証する。
 *
 * 実行後もこのファイルは記録として残す（削除しない）。
 * 参照: docs/changes/CHG-F006-XXX.md
 */
...
const MANIFEST_PATH = join(WORKSPACE, 'content', 'batches', 'F006', 'batch.json');
const SELECTION_PATH = join(WORKSPACE, 'content', 'batches', 'F006', 'source-snapshots', 'selection.json');
...
async function deriveRightsSnapshotIds(): Promise<readonly string[]> {
  ...
  if (record.kind !== 'f006-source-selection-snapshot' || record.batchId !== 'F006' ||
...
async function main(): Promise<void> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F006' || canonicalJson(checked.value) !== manifestText) {
    throw new Error(`既存batch.jsonがcanonical F006 manifestではありません: ${!checked.ok ? checked.error.code : 'text mismatch'}`);
  }
