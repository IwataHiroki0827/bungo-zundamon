...
/**
 * F009（夢野久作3作品追加）work単位の差分音声生成・容量forecast thin script。
 * f008-prepare-voice.tsをF009向けにパラメータ化した複製。CHG-F008-001（容量forecast
 * 自己増悪バグ、公開treeの成長に伴いplannedPagesBytesをcurrentPages.bytes全体で
 * 倍増する見積もりが閾値を自己増悪させる構造的欠陥）の是正済み固定上限方式
 * （`Math.min(currentPages.bytes, 150_000_000)`）をF009でも踏襲する。
 * @des DES-F009-008 @fun FUN-F009-009
 */

const BATCH_ID = 'F009';
const workIdArgument = process.argv[2];
if (!workIdArgument || !/^[0-9]{6}$/u.test(workIdArgument)) {
  throw new Error('6桁のwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f009-prepare-voice.ts 002381）');
}
...
/**
 * f009-prepare-editorial.tsのadvanceF009WorkStateと同じ再開可能パターンを
 * budget-approved/voicedへ拡張する。同一入力での再実行はstageRecordsの
 * 既存出力と照合し、一致すれば何もしない。
 */
async function advanceF009VoiceWorkState(
  workspace: string,
...
async function main(): Promise<void> {
  ...
  if (revisionArtifact.value.batchId !== BATCH_ID || revisionArtifact.value.workId !== WORK_ID || revisionArtifact.value.speech.length === 0) {
    throw new Error(`speech-revisions.jsonがF009/${WORK_ID}のtupleと一致しません`);
  }
  ...
  if (workProgress.status === 'reviewed') {
    ...
    if (forecast.result === 'blocked' || !forecast.canGenerate) {
      throw new Error(`F009 capacity forecast blocked: ${forecast.reasons.join(',')}`);
    }
    ...
    manifest = await advanceF009VoiceWorkState(
      workspace,
      manifest,
      'budget-approved',
      [forecastSha],
  ...
  if (generation.failed !== 0 || generation.succeeded !== authorizedPlan.uniqueAudioCount) {
    throw new Error('F009 voice generationが不完全です');
  }
