/**
 * F005/F006/F007と同型の不備（CHG-F005-078・F006/F007側是正参照）がF008にも再発した:
 * F008 batch.jsonはbatch level `stageRecords`が空のまま（F002〜F004が経由する
...
const MANIFEST_PATH = join(WORKSPACE, 'content', 'batches', 'F008', 'batch.json');
const SELECTION_PATH = join(WORKSPACE, 'content', 'batches', 'F008', 'source-snapshots', 'selection.json');
...
async function deriveRightsSnapshotIds(): Promise<readonly string[]> {
  ...
  if (record.kind !== 'f008-source-selection-snapshot' || record.batchId !== 'F008' ||
...
async function main(): Promise<void> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F008' || canonicalJson(checked.value) !== manifestText) {
    throw new Error(`既存batch.jsonがcanonical F008 manifestではありません: ${!checked.ok ? checked.error.code : 'text mismatch'}`);
  }
