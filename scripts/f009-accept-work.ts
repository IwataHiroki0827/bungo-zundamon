...
import { acceptF009Work, prepareF009WorkAcceptance } from '../src/content/f009-acceptance.ts';

/**
 * F009 work単位atomic受入CLI。src/content/f009-acceptance.tsのprepare/accept
...
async function main(): Promise<void> {
  ...
  const prepared = await prepareF009WorkAcceptance(workspace, MANIFEST_PATH, workIdArgument as WorkId);
  const result = await acceptF009Work(workspace, prepared, expectedManifestSha);
