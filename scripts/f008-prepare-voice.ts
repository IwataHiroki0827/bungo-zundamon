...
/**
 * F008（江戸川乱歩3作品追加）work単位の差分音声生成・容量forecast thin script。
 * f007-prepare-voice.tsをF008向けにパラメータ化した複製。DD-F008.md
 * FUN-F008-009記載のとおり、既存の汎用モジュール
 ...
 * を直接呼び出すだけで、F008専用の新規wrapperモジュールは作らない。
 *
 ...
 * content/batches/F008/work-artifacts/<workId>/.voice-stage-<workId>/ に保持する。
 *
 * work IDはCLI引数化済み（F007のprecedentを踏襲、3作品で再利用するため）。
 * @des DES-F008-008 @fun FUN-F008-009
 */

const BATCH_ID = 'F008';
const workIdArgument = process.argv[2];
if (!workIdArgument || !/^[0-9]{6}$/u.test(workIdArgument)) {
  throw new Error('6桁のwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f007-prepare-voice.ts 058126）');
}
...
const FORECAST_TOOL_VERSION = 'f008-prepare-voice-forecast-v1';
const VOICE_TOOL_VERSION = 'f008-prepare-voice-generate-v1';
...
/**
 * f008-prepare-editorial.tsのadvanceF008WorkStateと同じ再開可能パターンを
 * budget-approved/voicedへ拡張する。同一入力での再実行はstageRecordsの
 * 既存出力と照合し、一致すれば何もしない。
 */
async function advanceF008VoiceWorkState(
  workspace: string,
...
async function main(): Promise<void> {
  ...
  if (revisionArtifact.value.batchId !== BATCH_ID || revisionArtifact.value.workId !== WORK_ID || revisionArtifact.value.speech.length === 0) {
    throw new Error(`speech-revisions.jsonがF008/${WORK_ID}のtupleと一致しません`);
  }
  ...
  if (workProgress.status === 'reviewed') {
    ...
    if (forecast.result === 'blocked' || !forecast.canGenerate) {
      throw new Error(`F008 capacity forecast blocked: ${forecast.reasons.join(',')}`);
    }
    const forecastArtifact = {
      schemaVersion: '1.0.0' as const,
      kind: 'f007-voice-capacity-forecast' as const,
      batchId: BATCH_ID,
      workId: WORK_ID,
      expectedManifestSha: binding.expectedManifestSha,
      preTreeDigest: binding.preTreeDigest,
    ...
    manifest = await advanceF008VoiceWorkState(
      workspace,
      manifest,
      'budget-approved',
      [forecastSha],
  ...
  if (generation.failed !== 0 || generation.succeeded !== authorizedPlan.uniqueAudioCount) {
    throw new Error('F008 voice generationが不完全です');
  }
  ...
  const voiceEvidence = {
    schemaVersion: '1.0.0' as const,
    kind: 'f007-voice-evidence' as const,
    batchId: BATCH_ID,
    workId: WORK_ID,
