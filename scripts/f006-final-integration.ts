...
} from '../src/content/f006-catalog.ts';
...
/**
 * F006（中島敦3作品追加）の最終Catalog統合script。
 * F005の`f005-final-integration.ts`と同じ役割（3作品分の作品fragment片を結合し、
 * 単一の公開tree候補にする。固定v0.5.0 baselineとの差分がREQ-F006-001通りである
 * ことを検証する）を、F006既存の`mergeNewAuthorCatalog006`（FUN-F006-011、
 * baseline projection exact維持のassertを内包）・`deriveF006RouteSet`
 * （FUN-F006-012）を使って実装する。work単位の構築ロジックは
 * `scripts/f006-content-preview.ts`の`buildWorkFragmentPiece`と同型の複製
 * （同scriptはCLI引数化・`await main()`副作用付きのため直接importできない）。
 * @des DES-F006-002 DES-F006-009 DES-F006-010 @fun FUN-F006-002 FUN-F006-010 FUN-F006-011 FUN-F006-012
 */
...
/**
 * `scripts/f006-content-preview.ts`の`buildWorkFragmentPiece`と同一構造の複製
...
async function buildWorkFragmentPiece(
  ...
  if (dialogues.length !== candidatesArtifact.candidates.length || dialogues.length !== generation.assets.length) {
    throw new Error(`dialogue/candidate/audio件数が一致しません: ${workId}`);
  }
 ...
 * 維持され末尾へ中島敦分3作品が追加されたことを個別にcanonical比較する。
 * `scripts/f006-content-preview.ts`の`assertV050Invariant`と同一構造の複製
...
async function assertV050Invariant(
  ...
  if (
    ...
    catalog.works.length !== baselineCatalog.works.length + F006_WORKS.length ||
    ...
    throw new Error('v0.5.0固定4作者15作品projectionが最終Catalogと一致しません');
  }
  ...
  if (
    ...
    throw new Error('v0.5.0固定4作者分のartwork-provenances.jsonが最終buildと一致しません');
  }
}
...
async function main(): Promise<void> {
  ...
  if (process.argv.length !== (retainCandidate ? 3 : 2)) {
    throw new Error('usage: f006-final-integration.ts [--retain]');
  }
  ...
  if (!/^[a-f0-9]{40}$/u.test(sourceCommit) || status.trim() !== '') {
    throw new Error('F006最終統合にはexact clean source commitが必要です');
  }
  ...
  if (!checked.ok || checked.value.status !== 'accepted' ||
    ...
    throw new Error('F006全3作品がacceptedではありません');
  }
  ...
  for (const workId of manifest.workIds as readonly F006WorkId[]) {
    pieces.push(await buildWorkFragmentPiece(workspace, workId, snapshot, registry));
  }

  const fragment: F006CatalogFragment = {
    batchId: 'F006',
    feature: 'F006',
    authorId: '000119',
    authorArtwork: {
    ...
    works: pieces.map((piece) => piece.work) as F006CatalogFragment['works'],
  ...
  const finalCatalog = mergeNewAuthorCatalog006(v050.catalog, fragment, verifiedAuthor);
  const routeSet = deriveF006RouteSet(finalCatalog, STATIC_ROUTES);
  if (routeSet.routes.length !== 8) throw new Error('F006公開routeがexact 8件ではありません');
  ...
  const f006Fragment: BatchCatalogFragment = {
    authors: [{
      ...
      introducedByBatchId: 'F006' as const,
  ...
  const outputRoot = await mkdtemp(join(workspace, '.cache', 'f006-final-integration-'));
  let retained = false;
  try {
    const preparation = {
    ...
    const build = await buildIntegratedPublicTree(
      ...
      contentRoot,
      {
        ...
        batchCatalogs: { F002: f002, F003: f003, F004: f004, F005: f005, F006: f006Fragment },
    ...
    if (!validation.ok) throw new Error(`F006最終Catalogが不正です: ${validation.error.code}`);
