...
} from '../src/content/f007-catalog.ts';
import { loadPublishedV060Baseline, isMintedPublishedV060Baseline } from '../src/content/f007-baseline.ts';
import {
  buildF007SourceProvenance,
...
/**
 * F007（森鴎外3作品追加）の最終Catalog統合script。
 * F006の`f006-final-integration.ts`と同じ役割（3作品分の作品fragment片を結合し、
 * 単一の公開tree候補にする。固定v0.6.0 baselineとの差分がREQ-F007-001通りである
 * ことを検証する）を、F007既存の`mergeNewAuthorCatalog007`（FUN-F007-011、
 * baseline projection exact維持のassertを内包）・`deriveF007RouteSet`
 * （FUN-F007-012）を使って実装する。work単位の構築ロジックは
 * `scripts/f007-content-preview.ts`の`buildWorkFragmentPiece`と同型の複製
 * （同scriptはCLI引数化・`await main()`副作用付きのため直接importできない）。
 * @des DES-F007-002 DES-F007-009 DES-F007-010 @fun FUN-F007-002 FUN-F007-010 FUN-F007-011 FUN-F007-012
 */

const execFile = promisify(execFileCallback);
const BATCH_ID = 'F007';
const AUTHOR_ID = '000129';
const ARTWORK_PUBLIC_PATH = 'artwork/mori-ogai-zundamon.png';
const ARTWORK_SOURCE_PATH = `content/batches/${BATCH_ID}/public-files/artwork/mori-ogai-zundamon.png`;
// T-161（森鴎外作者画像生成、ComfyUI/FLUX.1 schnell実生成）でsealされた
// originalImageSha256（content/batches/F007/artwork-provenance.json記録値）。
const F007_ARTWORK_SHA256 = '05497a8c663d96c8152e4e815a38e16d4934b027ae0f5252acab2efd7f7ef0b8' as Sha256;
...
/**
 * `scripts/f007-content-preview.ts`の`buildWorkFragmentPiece`と同一構造の複製
 ...
 * 踏襲する。
 * @des DES-F007-002 DES-F007-010 @fun FUN-F007-002 FUN-F007-011
 */
async function buildWorkFragmentPiece(
  workspace: string,
  workId: F007WorkId,
  snapshot: Awaited<ReturnType<typeof rehydrateF007SelectionSnapshot>>,
  registry: ReturnType<typeof defineF007AuthorAndWorkRegistry>,
  ...
  const source = parseF007SourceRecord(workSnapshot, workId);
  const provenanceValue = buildF007SourceProvenance(source, snapshot);
  ...
  // (scripts/f007-content-preview.tsで発見・修正した同型の不変条件、
  // commit ee68be5参照)。正しい不変条件は「全candidateIdがassets側で
  // ちょうど1回ずつ言及される」こと。
...
/**
 * v0.6.0固定baselineの既存public file 968件を、統合buildの実tree（bytes/SHA）と
 * exact比較する。content/catalog.jsonとcontent/artwork-provenances.jsonは
 * F007追記により変化するため全体byte比較から除外し、既存分がprefixとして完全
 * 維持され末尾へ森鴎外分3作品が追加されたことを個別にcanonical比較する。
 * `scripts/f006-final-integration.ts`の`assertV050Invariant`と同一構造の複製
 * （最終統合ではaccumulatedWorkCountは常に3固定）。
 * @des DES-F007-002 @fun FUN-F007-002
 */
async function assertV060Invariant(
  build: IntegratedBuild,
  baseline: Awaited<ReturnType<typeof loadPublishedV060Baseline>>,
  workspace: string,
): Promise<void> {
  if (!isMintedPublishedV060Baseline(baseline)) {
    throw new Error('mint済みv0.6.0 baselineが必要です');
  }
  ...
  for (const expected of baseline.publicFiles) {
    ...
    if (!actual) throw new Error(`v0.6.0 baseline assetがpreviewにありません: ${expected.path}`);
    ...
    if (bytes.byteLength !== expected.bytes || sha256(bytes) !== actual.sha256) {
      throw new Error(`v0.6.0 baseline assetが変化しています: ${expected.path}`);
    }
  }
  ...
  if (
    ...
    catalog.works.length !== baselineCatalog.works.length + F007_WORKS.length ||
    ...
    throw new Error('v0.6.0固定5作者18作品projectionが最終Catalogと一致しません');
  }

  const expectedProvenanceFile = baseline.publicFiles.find((file) => file.path === 'content/artwork-provenances.json');
  if (!expectedProvenanceFile) throw new Error('v0.6.0 baselineにcontent/artwork-provenances.jsonがありません');
  ...
  if (baselineProvenanceBytes.byteLength !== expectedProvenanceFile.bytes) {
    throw new Error('v0.6.0 baselineのcontent/artwork-provenances.json参照元がpinned byte数と一致しません');
  }
  ...
  if (
    ...
    throw new Error('v0.6.0固定5作者分のartwork-provenances.jsonが最終buildと一致しません');
  }
}
...
async function main(): Promise<void> {
  ...
  if (process.argv.length !== (retainCandidate ? 3 : 2)) {
    throw new Error('usage: f007-final-integration.ts [--retain]');
  }
  ...
  if (!/^[a-f0-9]{40}$/u.test(sourceCommit) || status.trim() !== '') {
    throw new Error('F007最終統合にはexact clean source commitが必要です');
  }
  ...
  if (!checked.ok || checked.value.status !== 'accepted' ||
    ...
    throw new Error('F007全3作品がacceptedではありません');
  }
  ...
  const v060 = await loadPublishedV060Baseline(workspace);
  const registry = defineF007AuthorAndWorkRegistry();
  const verifiedAuthor = verifyF007AuthorIdentity(registry, v060.catalog);
  ...
  if (sha256(artworkBytes) !== F007_ARTWORK_SHA256) throw new Error('森鴎外作者画像SHAが固定値と一致しません');
  ...
  for (const workId of manifest.workIds as readonly F007WorkId[]) {
    pieces.push(await buildWorkFragmentPiece(workspace, workId, snapshot, registry, noticesByWorkId));
  }

  const fragment: F007CatalogFragment = {
    batchId: 'F007',
    feature: 'F007',
    authorId: AUTHOR_ID,
    authorArtwork: {
    ...
    works: pieces.map((piece) => piece.work) as F007CatalogFragment['works'],
    ...
    candidateCounts: {
      ...
      // 舞姫の外来語・術語タイポグラフィカル強調22件（NON_SPEECH判定）を含む、
      // 実際のcandidateTotal-publishedTotal差分をそのまま反映する
      // （scripts/f007-content-preview.tsの既存計算と同型）。
      editorialExcluded: pieces.reduce((sum, piece) => sum + (piece.candidateTotal - piece.publishedTotal), 0),
      audioExcluded: 0,
      editorialReasons: {
        NON_SPEECH: pieces.reduce((sum, piece) => sum + (piece.candidateTotal - piece.publishedTotal), 0),
      },
      audioFailureReasons: {},
    },
  ...
  const finalCatalog = mergeNewAuthorCatalog007(v060.catalog, fragment, verifiedAuthor);
  const routeSet = deriveF007RouteSet(finalCatalog, STATIC_ROUTES);
  if (routeSet.routes.length !== 9) throw new Error('F007公開routeがexact 9件ではありません');
  ...
  const f007Fragment: BatchCatalogFragment = {
    authors: [{
      ...
      introducedByBatchId: 'F007' as const,
  ...
  const outputRoot = await mkdtemp(join(workspace, '.cache', 'f007-final-integration-'));
  let retained = false;
  try {
    const preparation = {
    ...
    const [f001, f002, f003, f004, f005, f006, batches] = await Promise.all([
      ...
      loadPublishedF002CatalogFragment(workspace, v060.catalog),
      loadAcceptedF003CatalogFragment(workspace),
      loadPublishedF004CatalogFragment(workspace, v060.catalog),
      loadPublishedF005CatalogFragment(workspace, v060.catalog),
      loadPublishedF006CatalogFragment(workspace, v060.catalog),
    ...
    const publishedCatalogBatches = Object.fromEntries(v060.catalog.batches.map((batch) => [batch.batchId, batch]));
    ...
    const build = await buildIntegratedPublicTree(
      ...
      contentRoot,
      {
        ...
        batchCatalogs: { F002: f002, F003: f003, F004: f004, F005: f005, F006: f006, F007: f007Fragment },
        publishedCatalogBatches,
      },
      undefined,
      preparation,
    );
    await assertV060Invariant(build, v060, workspace);
    ...
    if (!validation.ok) throw new Error(`F007最終Catalogが不正です: ${validation.error.code}`);
