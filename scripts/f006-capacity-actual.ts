 ...
 *
 * F006のvoiced stage記録はvoiceEvidenceRefファイルのSHA-256だけを保持する間接ハッシュ
 * 方式（scripts/f006-prepare-voice.ts）であり、汎用promoteVerifiedWorkArtifacts
 * （src/content/batch-acceptance.ts）のassertInputsはvoiced stageのoutputHashesへ
 * generationDigest/completenessDigestを直接含むことを要求するため両者は噛み合わない。
 * ここではmanifestを直接操作し（transitionWorkStateは同一statusへの再遷移を許さない
 * ため使えない。既存src/content/batch-runtime.tsのappendActualEvidence（F002固有）と
 * 同型パターン）、
 *   1) voice-evidence/generation/completenessを再読込・再検証し、直接ハッシュ値を持つ
 *      追加のvoiced stage record（同じwork状態voicedのまま）をstageRecordsへ追記する
 *   2) 容量actual（pages dist previewの実vite build＋verifyActualCapacity）を実行する
...
const VOICED2_TOOL_VERSION = 'f006-capacity-actual-voiced-direct-v1';
const ACTUAL_TOOL_VERSION = 'f006-capacity-actual-v1';
...
async function loadManifest(workspace: string): Promise<BatchManifest> {
  ...
  if (!checked.ok || canonicalJson(checked.value) !== text || checked.value.batchId !== BATCH_ID) {
    throw new Error(`F006 manifestが不正です: ${checked.ok ? 'not canonical' : checked.error.code}`);
  }
...
async function main(): Promise<void> {
  ...
  const [voiceEvidence, generationCanonical, completeness, forecastArtifact] = await Promise.all([
    canonicalArtifact<{
      ...
      readonly kind: 'f006-voice-evidence';
  ...
  if (preview.value.mode !== 'work-preview' || preview.value.activeBatchId !== BATCH_ID || preview.value.activeWorkId !== WORK_ID) {
    throw new Error('content-preview.jsonが現在のwork-previewと一致しません（scripts/f006-content-preview.tsを先に実行してください）');
  }
  ...
  const pagesOutputRoot = join(workspace, '.cache', `.f006-actual-pages-${WORK_ID}-${Date.now()}`);
  ...
  try {
    ...
    if (actual.result === 'blocked') throw new Error(`F006容量actualがblockedです: ${actual.reasons.join(',')}`);
