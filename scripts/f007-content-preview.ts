...
/**
 * F007（森鴎外3作品追加）work単位のwork-preview content統合script。
 * F006のscripts/f006-content-preview.tsをF007向けにパラメータ化した複製。
 * baselineをv0.6.0（F001〜F006公開済み、5作者18作品）へ拡張する。
 * F006との違いは、舞姫の作品固有notice（QA-F007 No.4、official-content-warning）を
 * content/batches/F007/work-notices.json（T-161で新規作成済み）から
 * loadAndValidateWorkNoticesで実データ取得し、work.noticesへ実際に反映する点
 * （F006はdialogue-excerpt-scope固定のみでnoticesを空扱いしていたため異なる）。
...
if (!workIdArgument || !F007_WORKS.some((work) => work.workId === workIdArgument)) {
  throw new Error(
    `F007_WORKSに定義済みのwork IDを引数で指定してください（例: node --experimental-transform-types scripts/f007-content-preview.ts 058126）: ${String(workIdArgument)}`,
  );
}
...
const ARTWORK_SHA256 = '05497a8c663d96c8152e4e815a38e16d4934b027ae0f5252acab2efd7f7ef0b8' as Sha256;
...
async function assertV060Invariant(
  ...
  if (
    ...
    throw new Error('v0.6.0固定5作者18作品projectionがpreview catalogと一致しません');
  }
  ...
  if (
    ...
    throw new Error('v0.6.0固定5作者分のartwork-provenances.jsonがpreviewと一致しません');
  }
}
...
async function buildWorkFragmentPiece(
  ...
  // F007は舞姫等の候補にNON_SPEECH（外来語・術語のタイポグラフィカルな強調）で
  // rejected判定された候補が含まれるため、reviews（approved/rejected両方）のうち
  ...
  return {
    work: {
      ...
      // DD-F007.md FUN-F007-010訂正どおり、舞姫の公式「不適切な表現」notice
      // （QA-F007 No.4）を含むwork固有notice集合をcontent/batches/F007/
...
async function main(): Promise<void> {
  ...
  if (sha256(artworkBytes) !== ARTWORK_SHA256) throw new Error('森鴎外作者画像SHAが固定値と一致しません');
  ...
  const accumulatedWorkIds = manifest.workIds.slice(0, workIndex + 1) as readonly F007WorkId[];
  ...
  const currentFragment: BatchCatalogFragment = {
    authors: [{
      ...
      artwork: { path: ARTWORK_PUBLIC_PATH, alt: '森鴎外をイメージしたずんだもん', sha256: sha256(artworkBytes) },
  ...
  const activeStage = join(workspace, '.cache', `.f007-active-${randomUUID()}`);
  const previewStage = join(workspace, '.cache', `.f007-preview-${randomUUID()}`);
  ...
  const build = await buildIntegratedPublicTree(
    ...
    previewStage,
    {
      ...
      batchCatalogs: { F002: f002, F003: f003, F004: f004, F005: f005, F006: f006 },
      publishedCatalogBatches,
    },
    active,
  );
  await assertV060Invariant(build, v060, workspace, accumulatedWorkIds.length);
