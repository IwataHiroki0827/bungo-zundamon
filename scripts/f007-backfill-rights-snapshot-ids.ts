/**
 * F005/F006と同型の不備（CHG-F005-078・F006側是正参照）がF007にも再発した:
 * F007 batch.jsonはbatch level `stageRecords`が空のまま（F002〜F004が経由する
 ...
 * 選定時2026-08-22に取得・検証済み）から機械的に値を導出し、batch.jsonへ
...
const MANIFEST_PATH = join(WORKSPACE, 'content', 'batches', 'F007', 'batch.json');
const SELECTION_PATH = join(WORKSPACE, 'content', 'batches', 'F007', 'source-snapshots', 'selection.json');
...
async function deriveRightsSnapshotIds(): Promise<readonly string[]> {
  ...
  // F007のselection.jsonはF006側のscript（persistF007SourceSnapshot等が
  // f007-source.tsへF006相当の構造をそのまま複製実装したため）由来のkind文字列
  // 'f006-source-selection-snapshot'を持つ（batchId自体は正しくF007）。
  if (record.kind !== 'f006-source-selection-snapshot' || record.batchId !== 'F007' ||
...
async function main(): Promise<void> {
  ...
  if (!checked.ok || checked.value.batchId !== 'F007' || canonicalJson(checked.value) !== manifestText) {
    throw new Error(`既存batch.jsonがcanonical F007 manifestではありません: ${!checked.ok ? checked.error.code : 'text mismatch'}`);
  }
