...
- 対象仕様: `docs/tests/ut/UT-F004.md`
- 対応表の実行コード: `src/content/f004-spec-coverage.test.ts` の `UT_MAP`
- 対象ID: `UT-F004-001`〜`UT-F004-037`
- 対応済み: **37 / 37**
- 未対応・余剰・自動化不可: **0**

| UT ID | 対応する主な自動試験 |
|---|---|
| 001〜005 | 承認文脈、固定baseline、候補選定 |
| 006〜007 | 取得元・権利証跡 |
| 008〜011 | 編集・抽出・正規化 |
| 012〜017 | 音声生成・音声成果物 |
| 018〜020 | 作品単位の受入 |
| 021〜024、037 | Catalog統合・公開木構築 |
| 025〜030 | お気に入り状態・UI・起動統合 |
| 031〜036 | runtime受入・境界防御・release check |

仕様ID集合と対応表のキー集合をテスト内で完全一致させ、対応先ファイルが実在することも検証する。正式PASS/FAILはF004のUT attempt生ログと結果文書で判定する。
