...
phase: requirements
feature: F005
updated: 2026-07-29T04:24:00+09:00
next_actions:
  - "F005で知名度順の次作者と代表3作品を一次情報から選定し、要求・適格性試験仕様を作成する"
blocked_by: []
---
...
- T-063を開始し、知名度順の次作者を夏目漱石（人物ID`000148`）とした。公式作者一覧・図書カード・XHTML・文化庁資料を確認し、容量上限内の候補を「夢十夜」「倫敦塔」「趣味の遺伝」に絞った。公式XHTML概算は229候補・発話7,330文字、安全側音声予測72,589,906 bytesで、QA未回答は0件である。

## 直近の作業（最新5件）

- T-063で夏目漱石と候補3作品を一次情報から選定し、権利・台詞密度・容量のドメイン調査とQA回答を作成
...
- DOMAIN-F005とQA-F005を根拠に、公開済みv0.4.0を固定baselineとするSRS-F005・QT-F005を作成する。
- 要求と適格性試験の独立レビュー後、本会話の包括承認をゲート①へ記録する。

## 未解決事項

- C:の空きは公開直前実測で62,280,704,000 bytes。F004必要526,889,524 bytesと5 GiB停止基準に十分な余裕があり、F005の音声生成前にも再計測する。
